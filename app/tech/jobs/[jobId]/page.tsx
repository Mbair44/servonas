...
export default async function TechnicianJobDetail({ params, searchParams }: {
  ...
  return <main className="tech-shell"><header className="tech-detail-header"><Link href="/tech">← Today’s jobs</Link><span className={`job-status ${job.status}`}>{job.status.replaceAll("_", " ")}</span></header>
    ...
    <section className="tech-hero"><small>Job #{job.job_number}</small><h1>{job.title}</h1><p>{service?.name || "Custom work"} · {job.priority} priority</p><TechnicianLiveLocation jobId={jobId} status={job.status}/>{action&&job.status!=="dispatched"?<form action={transitionTechnicianJob.bind(null,jobId)}><input type="hidden" name="status" value={action.status}/><button className="sv-button sv-full">{action.label}</button></form>:!action&&<p className="tech-status-help">{job.status==="completed"?"Work completed":job.status==="scheduled"||job.status==="confirmed"?"Waiting for office dispatch":""}</p>}</section>
