...
export default async function TechnicianHome({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="tech-shell"><header className="tech-header"><div><span className="sv-kicker">Servonas Technician</span><h1>Hello, {profiles[0].display_name}</h1><p>{profiles.map((profile) => relation(profile.businesses)?.name).filter(Boolean).join(" · ")}</p><Link className="sv-button sv-secondary" href={`/tech/route?business=${profiles[0].business_id}`}>View today’s route</Link></div><form action={signOut}><button className="text-button">Log out</button></form></header>{query.error&&<div className="workspace-notice error">{query.error}</div>}
