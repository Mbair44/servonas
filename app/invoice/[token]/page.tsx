...
import InvoicePaymentIcon from "@/components/InvoicePaymentIcon";
...
export default async function PublicInvoice({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<{payment?:string}>}){
  ...
  const {data:invoice,error}=await supabase.from("invoices").select(
    "*,businesses(name,timezone),customers!invoices_customer_fk(first_name,last_name,company_name,email,tax_exempt,tax_exemption_reference),service_locations!invoices_location_fk(location_name,street_address,city,state,postal_code),jobs!invoices_job_fk(id,job_number,title,starts_at,status)"
  ...
  const [{data:lines,error:linesError},{data:fees,error:feesError},{data:payments,error:paymentsError},{data:settings},{data:paymentAccount},{data:billingOptions}]=await Promise.all([
    ...
    supabase.from("business_billing_settings").select("accept_online_card,accept_cash,accept_check,accept_pay_by_phone,check_payable_to,payment_phone").eq("business_id",invoice.business_id).maybeSingle(),
  ]);
  ...
  const cardAllowed=(billingOptions?.accept_online_card??true)&&onlineReady;
  const cashAllowed=Boolean(billingOptions?.accept_cash);
  const checkAllowed=Boolean(billingOptions?.accept_check);
  const phoneAllowed=Boolean(billingOptions?.accept_pay_by_phone&&billingOptions?.payment_phone);
  const hasOfflineOptions=cashAllowed||checkAllowed||phoneAllowed;
  const hasPaymentOptions=cardAllowed||hasOfflineOptions;
  ...
  const issuedLabel=invoice.issue_date?formatBusinessDate(`${invoice.issue_date}T12:00:00Z`,timeZone):"Not specified";
  const dueLabel=invoice.due_date?formatBusinessDate(`${invoice.due_date}T12:00:00Z`,timeZone):"Upon receipt";
  const dueDays=invoice.due_date?Math.ceil((new Date(`${invoice.due_date}T23:59:59Z`).getTime()-Date.now())/86_400_000):null;
  const dueStatus=Number(invoice.balance_due_cents)<=0?"Paid":dueDays===null?"Due upon receipt":dueDays<0?`${Math.abs(dueDays)} day${Math.abs(dueDays)===1?"":"s"} overdue`:dueDays===0?"Due today":`Due in ${dueDays} day${dueDays===1?"":"s"}`;
  return <main className="public-estimate public-invoice" style={{"--estimate-brand":settings?.brand_color||"#4f46e5"} as React.CSSProperties}>
    <article className="public-estimate-document" aria-labelledby="invoice-title">
      ...
      <section className="public-invoice-hero"><div className="public-estimate-title"><span className={`invoice-due-badge ${dueDays!==null&&dueDays<0?"overdue":""}`}>{dueStatus}</span><h1 id="invoice-title">{invoice.title}</h1><p>{invoice.customer_notes||"Invoice details and payment history are shown below."}</p></div><aside><span>Amount due</span><strong>{formatCents(invoice.balance_due_cents,invoice.currency)}</strong><small>Due date</small><b>{dueLabel}</b></aside></section>
      <section className="public-estimate-parties" aria-label="Invoice parties and dates"><div><span>Bill to</span><strong>{customerName}</strong><small>{customer?.email}</small>{location&&<small>{location.location_name} · {location.street_address}, {location.city}, {location.state} {location.postal_code}</small>}</div><div><span>Invoice details</span><dl><div><dt>Invoice #</dt><dd>{invoice.invoice_number}</dd></div><div><dt>Issued</dt><dd>{issuedLabel}</dd></div><div><dt>Due date</dt><dd>{dueLabel}</dd></div></dl></div></section>
      ...
      <dl className="public-estimate-totals"><div><dt>Subtotal</dt><dd>{formatCents(invoice.subtotal_cents,invoice.currency)}</dd></div>{invoice.discount_total_cents>0&&<div><dt>Discount</dt><dd>−{formatCents(invoice.discount_total_cents,invoice.currency)}</dd></div>}<div><dt>Taxable subtotal</dt><dd>{formatCents(invoice.taxable_subtotal_cents??0,invoice.currency)}</dd></div><div><dt>Sales tax ({(Number(invoice.tax_rate_basis_points??0)/100).toFixed(2)}%)</dt><dd>{formatCents(invoice.tax_total_cents,invoice.currency)}</dd></div>{(fees??[]).map(fee=><div key={fee.id}><dt>{fee.name_snapshot}</dt><dd>{formatCents(fee.amount_cents,invoice.currency)}</dd></div>)}<div><dt>Total</dt><dd>{formatCents(invoice.grand_total_cents,invoice.currency)}</dd></div><div><dt>Amount paid</dt><dd>−{formatCents(invoice.amount_paid_cents-invoice.amount_refunded_cents,invoice.currency)}</dd></div><div className="total"><dt>Total due</dt><dd>{formatCents(invoice.balance_due_cents,invoice.currency)}</dd></div></dl>
      {customer?.tax_exempt&&<section className="public-estimate-closed"><strong>Tax-exempt customer</strong><p>{customer.tax_exemption_reference?`Reference: ${customer.tax_exemption_reference}`:"Sales tax was not charged on this invoice."}</p></section>}
      ...
      {invoice.balance_due_cents>0&&!["void","refunded"].includes(invoice.status)&&hasPaymentOptions&&<section className="public-invoice-payment" id="payment-options"><header><span><InvoicePaymentIcon name="payment"/></span><h2>Payment options</h2></header><div className={`public-invoice-payment-grid ${cardAllowed&&hasOfflineOptions?"":"single"}`}>{cardAllowed&&<div><h3><InvoicePaymentIcon name="card"/> Pay online</h3><p>Pay securely online using your credit or debit card.</p><div className="stripe-payment-note"><b><InvoicePaymentIcon name="lock"/></b><span><strong>Secure payment powered by Stripe</strong><small>Accepted major credit and debit cards.</small></span></div><InvoicePaymentForm token={token} balanceDueCents={Number(invoice.balance_due_cents)} depositRemainingCents={depositRemaining} allowPartialPayments={Boolean(invoice.allow_partial_payments)} minimumPartialPaymentCents={Number(invoice.minimum_partial_payment_cents)} currency={invoice.currency}/></div>}{hasOfflineOptions&&<div className="public-invoice-offline"><h3>Other ways to pay</h3>{cashAllowed&&<p><b><i><InvoicePaymentIcon name="cash"/></i> Cash</b></p>}{checkAllowed&&<p><b><i><InvoicePaymentIcon name="check"/></i> Check</b><small>Make payable to {billingOptions?.check_payable_to||business?.name}</small></p>}{phoneAllowed&&<p><b><i><InvoicePaymentIcon name="phone"/></i> Pay by phone</b><small>Call {billingOptions?.payment_phone} to pay with a card</small></p>}</div>}</div></section>}
      {!!payments?.length&&<section className="public-invoice-history" aria-labelledby="payment-history-heading"><h2 id="payment-history-heading">Payment history and receipts</h2><div>{payments.map(payment=><article key={payment.id}><div><strong>{formatCents(payment.amount_cents,payment.currency)}</strong><span>{(payment.payment_method_type||payment.provider).replaceAll("_"," ")}</span></div><div><strong>Payment received</strong><span>{formatBusinessDateTime(payment.paid_at||payment.received_at||payment.created_at,timeZone)}</span>{payment.offline_reference&&<small>Reference: {payment.offline_reference}</small>}{payment.provider_receipt_url&&<a href={payment.provider_receipt_url} target="_blank" rel="noreferrer">View Stripe receipt</a>}</div></article>)}</div></section>}
      ...
      <section className="public-invoice-secure"><b aria-hidden="true">♢</b><div><strong>Secure invoice</strong><small>This is a secure invoice access page. Your payment information is encrypted and processed by Stripe.</small></div></section>
