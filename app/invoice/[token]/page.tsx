...
export default async function PublicInvoice({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<{payment?:string}>}){
  ...
  return <main className="public-estimate public-invoice" style={{"--estimate-brand":settings?.brand_color||"#4f46e5"} as React.CSSProperties}>
    <article className="public-estimate-document" aria-labelledby="invoice-title">
      ...
      {invoice.balance_due_cents>0&&!["void","refunded"].includes(invoice.status)&&<section className="public-invoice-payment" id="payment-options"><header><span aria-hidden="true">▣</span><h2>Payment options</h2></header>{onlineReady?<div className="public-invoice-payment-grid"><div><h3>Pay online</h3><p>Pay securely using your credit or debit card.</p><div className="payment-card-brands" aria-label="Major cards accepted"><span>VISA</span><span>●</span><span>AMEX</span><span>DISCOVER</span></div><InvoicePaymentForm token={token} balanceDueCents={Number(invoice.balance_due_cents)} depositRemainingCents={depositRemaining} allowPartialPayments={Boolean(invoice.allow_partial_payments)} minimumPartialPaymentCents={Number(invoice.minimum_partial_payment_cents)} currency={invoice.currency}/></div><div className="public-invoice-offline"><h3>Other ways to pay</h3><p><b>Cash or check</b><small>Contact {business?.name} to arrange payment.</small></p></div></div>:<><strong>Online payment is not currently available.</strong><p>Contact {business?.name} to arrange payment.</p></>}</section>}
