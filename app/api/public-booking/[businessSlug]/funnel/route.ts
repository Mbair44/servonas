...
const sessionMetricUpdate=(metadata:Record<string,unknown>)=>({
 total_session_duration_seconds:wholeNumber(metadata.session_duration_increment_seconds),
 engaged_duration_seconds:wholeNumber(metadata.engaged_duration_increment_seconds),
});
...
export async function POST(request:Request,{params}:{params:Promise<{businessSlug:string}>}){
 ...
 if(body.touchSession||body.touchOnly){
  const nowIso=new Date().toISOString(),attribution=body.attribution??{},sessionPath=clean(body.path,1000)||null,sessionPageType=pageType(body.pageType),metricUpdate=sessionMetricUpdate(metadata),first:Record<string,unknown>={id:sessionId,business_id:businessId,first_landing_url:clean(body.landingUrl,2000)||null,first_landing_path:sessionPath,first_referrer:clean(body.referrer,2000)||null,last_seen_at:nowIso,updated_at:nowIso,session_started_at:nowIso,session_ended_at:null,entry_path:sessionPath,last_path:sessionPath,entry_page_type:sessionPageType,last_page_type:sessionPageType,total_session_duration_seconds:metricUpdate.total_session_duration_seconds,engaged_duration_seconds:metricUpdate.engaged_duration_seconds,page_count:body.event==="landing_page_view"||body.event==="landing_view"?1:0,engaged_page_count:["service_view","inventory_view","inventory_item_view","rental_viewed","available_inventory_viewed"].includes(event)?1:0,browser:textValue(metadata.browser),operating_system:textValue(metadata.operating_system),device_type:textValue(metadata.device_type)};
  for(const key of attributionKeys)first[key]=clean(attribution[key],500)||null;
  const {data:existing,error:existingError}=await db.from("booking_attribution_sessions").select("id,page_count,engaged_page_count,total_session_duration_seconds,engaged_duration_seconds").eq("business_id",businessId).eq("id",sessionId).maybeSingle();
  ...
  const sessionRow=existing?{last_seen_at:nowIso,updated_at:nowIso,session_ended_at:null,last_path:sessionPath,last_page_type:sessionPageType,browser:textValue(metadata.browser),operating_system:textValue(metadata.operating_system),device_type:textValue(metadata.device_type),page_count:Math.max(0,Number(existing.page_count??0))+pageIncrement,engaged_page_count:Math.max(0,Number(existing.engaged_page_count??0))+engagedIncrement,total_session_duration_seconds:Math.max(0,Number(existing.total_session_duration_seconds??0))+metricUpdate.total_session_duration_seconds,engaged_duration_seconds:Math.max(0,Number(existing.engaged_duration_seconds??0))+metricUpdate.engaged_duration_seconds}:first;
  ...
  logStage("Booking funnel session upsert completed",{stage:"session_upsert",businessId,businessSlug,sessionId,event,source:normalizeMarketingSource(attribution),hasGclid:Boolean(attribution.gclid||attribution.gbraid||attribution.wbraid),hasFbclid:Boolean(attribution.fbclid),touchSession:Boolean(body.touchSession),touchOnly:Boolean(body.touchOnly),pageType:sessionPageType,totalSessionDurationSeconds:metricUpdate.total_session_duration_seconds,engagedDurationSeconds:metricUpdate.engaged_duration_seconds});
 }
