...
export async function POST(request:Request,{params}:{params:Promise<{businessSlug:string}>}){
 ...
 if(body.touchSession||body.touchOnly){
  ...
  const nextInteractionCount=Math.max(0,Number(existing?.meaningful_interaction_count??0))+(interaction.occurred?1:0);
  const sessionRow=existing?{last_seen_at:nowIso,updated_at:nowIso,session_ended_at:metricUpdate.finalFlushReceived?nowIso:null,last_path:sessionPath,last_page_type:sessionPageType,browser:textValue(metadata.browser),operating_system:textValue(metadata.operating_system),device_type:textValue(metadata.device_type),page_count:Math.max(0,Number(existing.page_count??0))+pageIncrement,engaged_page_count:Math.max(0,Number(existing.engaged_page_count??0))+engagedIncrement,total_session_duration_seconds:Math.round(nextMilliseconds/1000),engaged_duration_seconds:Math.round(nextMilliseconds/1000),total_session_duration_milliseconds:metricUpdate.incrementMilliseconds==null?existing.total_session_duration_milliseconds:nextMilliseconds,engaged_duration_milliseconds:metricUpdate.incrementMilliseconds==null?existing.engaged_duration_milliseconds:nextMilliseconds,duration_source:metricUpdate.source??existing.duration_source??null,duration_final_flush_received:Boolean(existing.duration_final_flush_received)||metricUpdate.finalFlushReceived,duration_last_flush_reason:metricUpdate.flushReason??existing.duration_last_flush_reason??null,first_interaction_type:existing.first_interaction_type??(interaction.occurred?interaction.type:null),first_interaction_label:existing.first_interaction_type?undefined:(interaction.occurred?interaction.label:null),first_interaction_identifier:existing.first_interaction_type?undefined:(interaction.occurred?interaction.identifier:null),first_interaction_path:existing.first_interaction_type?undefined:(interaction.occurred?interaction.path:null),first_interaction_at:existing.first_interaction_type?undefined:(interaction.occurred?nowIso:null),time_to_first_interaction_milliseconds:existing.first_interaction_type?undefined:(interaction.occurred?interaction.milliseconds:null),meaningful_interaction_count:nextInteractionCount,automated_classification:existing.automated_classification==="automated_likely"?"automated_likely":automation.classification,automated_classification_reason:existing.automated_classification==="automated_likely"?existing.automated_classification_reason:(automation.reason??existing.automated_classification_reason??null)}:first;
  // The initial page event and heartbeat can race before either observes the
  // new session. This is the composite key used by the session/event FK.
  // DO NOTHING preserves the winning request's first-touch attribution.
  const sessionWrite=existing?db.from("booking_attribution_sessions").update(sessionRow).eq("business_id",businessId).eq("id",sessionId):db.from("booking_attribution_sessions").upsert(sessionRow,{onConflict:"business_id,id",ignoreDuplicates:true});
  ...
  if(!existing){const {error:raceUpdateError}=await db.from("booking_attribution_sessions").update({last_seen_at:nowIso,updated_at:nowIso,session_ended_at:metricUpdate.finalFlushReceived?nowIso:null,last_path:sessionPath,last_page_type:sessionPageType,browser:textValue(metadata.browser),operating_system:textValue(metadata.operating_system),device_type:textValue(metadata.device_type),duration_source:metricUpdate.source,duration_final_flush_received:metricUpdate.finalFlushReceived,duration_last_flush_reason:metricUpdate.flushReason}).eq("business_id",businessId).eq("id",sessionId);if(raceUpdateError){console.error("Booking attribution session race update failed",{stage:"session_race_update",businessId,businessSlug,sessionId,event,code:raceUpdateError.code,message:raceUpdateError.message});diagnosticRequest(body,businessSlug,"rejected",{reason:"session_race_update_failed"});return diagnosticResponse("session_race_update_failed");}}
 ...
 if(!body.touchSession){
  ...
  const {error:parentError}=await db.from("booking_attribution_sessions").upsert(sessionSeed,{onConflict:"business_id,id",ignoreDuplicates:true});
