...
export async function GET(request:Request,{params}:{params:Promise<{businessSlug:string}>}){
 ...
 const {data:settings}=await db.from("booking_settings").select("business_id,buffer_minutes").ilike("public_slug",businessSlug).eq("enabled",true).maybeSingle();
 if(!settings)return NextResponse.json({error:"Booking page not found."},{status:404});
 const [{data:items},{data:blocked},{data:reserved,error}]=await Promise.all([
 ...
 return NextResponse.json({availability:Object.fromEntries((items??[]).map(item=>[item.id,blockedIds.has(item.id)?0:Math.max(0,Number(item.stock_quantity)-Number(used.get(item.id)??0))])),bufferMinutes:buffer},{headers:{"Cache-Control":"no-store"}});
}
