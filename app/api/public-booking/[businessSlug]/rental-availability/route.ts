...
async function loadCalendarAvailability({db,businessId,timezone,itemId,startDate,endDate,requestedQuantity,bufferMinutes,rentalDurationMinutes}:{db:NonNullable<ReturnType<typeof getSupabaseAdmin>>;businessId:string;timezone:string;itemId:string;startDate:string;endDate:string;requestedQuantity:number;bufferMinutes:number;rentalDurationMinutes:number}){
 ...
 if(reservationError||blackoutError)return {error:"Availability could not be checked.",status:500 as const};
 const reservations:RentalReservationWindow[]=[];
 for(const row of reservationRows??[]){const booking=Array.isArray(row.bookings)?row.bookings[0]:row.bookings;if(!booking)continue;const startsAt=booking.rental_starts_at?new Date(booking.rental_starts_at):zonedDateTimeToUtc(row.rental_date,time(booking.event_start_time),timezone),endsAt=booking.rental_ends_at?new Date(booking.rental_ends_at):zonedDateTimeToUtc(row.rental_date,time(booking.event_end_time),timezone);reservations.push({startsAt,endsAt,quantity:Number(row.quantity||0)});}
