...
export default async function PublicLandingPage({params}:{params:Promise<{siteSlug:string;promotionSlug:string}>}){
 ...
 const {data:website}=await db.from("business_website_settings").select("business_id,meta_pixel_id,google_reviews,photo_urls,request_service_enabled,booking_enabled").ilike("public_slug",siteSlug).eq("status","published").maybeSingle();if(!website)notFound();
