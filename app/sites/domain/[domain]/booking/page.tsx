...
export default async function CustomDomainBookingPage({params,searchParams}:{params:Promise<{domain:string}>;searchParams:Promise<{error?:string;embed?:string;item?:string;sv_at?:string;checkout?:string;checkoutUrl?:string}>}){
 ...
 return (
  <>{embedded&&<EmbeddedBookingBridge/>}<TenantBookingFunnelTracker businessSlug={record.site.bookingSlug} initialSessionId={query.sv_at}/><main className={`public-booking${embedded?" embedded-booking":""}`} style={{"--booking-brand":settings.brand_color} as React.CSSProperties}>
   <section className="public-booking-card">
    ...
    {isPartyRental?(
     ...
     <PublicBookingForm action={submitPublicBooking.bind(null,record.site.bookingSlug)} services={services} schedule={schedule} collectAddress={Boolean(settings.collect_address)} intakeQuestions={settings.intake_questions??[]} businessName={businessName??"this business"} maximumDaysAhead={Number(settings.maximum_days_ahead??60)} googleMapsApiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY} publicSlug={record.site.bookingSlug} timezone={settings.timezone??"America/Phoenix"}/>
    )}
   </section>
