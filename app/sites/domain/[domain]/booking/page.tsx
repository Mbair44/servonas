...
export default async function CustomDomainBookingPage({params,searchParams}:{params:Promise<{domain:string}>;searchParams:Promise<{error?:string;embed?:string;item?:string;promotion?:string;promotionId?:string;returnTo?:string;sv_at?:string;checkout?:string;checkoutUrl?:string}>}){
 ...
 return (
  <>{record.kind==="ok"&&record.site.metaPixelId&&<TenantMetaPixel pixelId={record.site.metaPixelId}/>} {embedded&&<EmbeddedBookingBridge/>}<TenantBookingFunnelTracker businessSlug={bookingSlug} initialSessionId={query.sv_at}/><main className={`public-booking${embedded?" embedded-booking":""}`} style={{"--booking-brand":settings.brand_color} as React.CSSProperties}>
   ...
   {!embedded&&<footer><Link href={`/book/${bookingSlug}/privacy`}>Privacy Policy</Link> · <Link href={`/book/${bookingSlug}/terms`}>Text Messaging Terms</Link></footer>}
  </main></>
 );
}
