...
export default async function CustomDomainBookingPage({params,searchParams}:{params:Promise<{domain:string}>;searchParams:Promise<{error?:string;embed?:string;item?:string;promotion?:string;promotionId?:string;returnTo?:string;sv_at?:string;checkout?:string;checkoutUrl?:string}>}){
 ...
 const data=await loadPublicBookingData(bookingSlug,query.promotion,query.promotionId);
 if(!data)notFound();
 ...
 return (
  <>{record.kind==="ok"&&record.site.metaPixelId&&<TenantMetaPixel pixelId={record.site.metaPixelId}/>} {embedded&&<EmbeddedBookingBridge/>}<TenantBookingFunnelTracker businessSlug={bookingSlug} initialSessionId={query.sv_at}/><main className={`public-booking${embedded?" embedded-booking":""}`} style={{"--booking-brand":settings.brand_color} as React.CSSProperties}>
   <section className="public-booking-card">
    ...
    {isPartyRental?(
     rentalInventory.length?<PartyRentalBookingClient fullDayRentalMessage={data.fullDayRentalMessage} cancellationPolicy={data.cancellationPolicy} businessSlug={bookingSlug} businessName={businessName??"this business"} inventory={rentalInventory} capacityByItem={rentalCapacity} blockedDates={rentalBlockedDates} relatedItems={rentalUpsells} schedule={schedule} timezone={settings.timezone??"America/Phoenix"} standardDurationMinutes={Number(settings.rental_duration_minutes??240)} standardRentalHours={Number(settings.standard_rental_hours??24)} allowMultiDay={Boolean(settings.allow_multi_day_rentals)} additionalDayPricingType={settings.additional_day_pricing_type??"full_price"} additionalDayDiscountPercent={Number(settings.additional_day_discount_percent??0)} additionalDayFlatRateCents={settings.additional_day_flat_rate_cents==null?null:Number(settings.additional_day_flat_rate_cents)} maxRentalDays={settings.max_rental_days==null?null:Number(settings.max_rental_days)} depositPercent={Number(settings.rental_deposit_percent??25)} onlinePaymentsReady={rentalOnlinePaymentsReady} googleMapsApiKey={googleMapsApiKey} initialItemId={query.item&&rentalInventory.some(item=>item.id===query.item)?query.item:undefined} initialPromotionCode={query.promotion} attributionSessionId={query.sv_at} initialCheckout={query.checkout==="1"} checkoutUrl={query.checkoutUrl??"/booking/checkout"} catalogUrl="/booking" policyBasePath=""/>:query.promotion?<div className="booking-empty"><h2>We couldn&apos;t load this offer&apos;s rentals.</h2><p>Return to the offer and try again, or choose another promotional rental.</p><a className="button" href={query.returnTo||"/"}>Back to the offer</a></div>:<div className="booking-empty">No rental items are available for online booking yet.</div>
    ):!services?.length?(
     <div className="booking-empty">No services are available for online booking yet.</div>
    ):(
     <PublicBookingForm action={submitPublicBooking.bind(null,bookingSlug)} services={services} schedule={schedule} collectAddress={Boolean(settings.collect_address)} intakeQuestions={settings.intake_questions??[]} businessName={businessName??"this business"} maximumDaysAhead={Number(settings.maximum_days_ahead??60)} googleMapsApiKey={googleMapsApiKey} publicSlug={bookingSlug} timezone={settings.timezone??"America/Phoenix"} embedded={embedded} policyBasePath=""/>
    )}
   </section>
   {!embedded&&<footer><Link href="/privacy">Privacy Policy</Link> · <Link href="/terms">Terms of Service</Link></footer>}
  </main></>
 );
}
