...
export async function purchaseRequestedDomain(data:FormData){
 ...
 const registrant:VercelRegistrant={firstName:text(data,"firstName"),lastName:text(data,"lastName"),email:text(data,"email"),phone,address1:text(data,"address1"),address2:text(data,"address2")||undefined,city:text(data,"city"),state:text(data,"state"),zip:text(data,"zip"),country,companyName:text(data,"companyName")||undefined};
 if(!registrant.firstName||!registrant.lastName||!/^\S+@\S+\.\S+$/.test(registrant.email)||!/^\+[1-9]\d{7,14}$/.test(registrant.phone)||!registrant.address1||!registrant.city||!registrant.state||!registrant.zip||!/^[A-Z]{2}$/.test(registrant.country))redirect(destination("error","Complete the registrant contact information. Phone formatting is handled automatically."));
 if(process.env.GOOGLE_MAPS_API_KEY){const verified=await resolveGoogleAddress({line1:registrant.address1,line2:registrant.address2,city:registrant.city,region:registrant.state,postalCode:registrant.zip,countryCode:registrant.country});if(verified.status!=="verified"||!verified.normalizedAddress)redirect(destination("error","Google could not verify the registrant address. Confirm the street, city, state, and ZIP and try again."));registrant.address1=verified.normalizedAddress.line1??registrant.address1;registrant.address2=registrant.address2??verified.normalizedAddress.line2??undefined;registrant.city=verified.normalizedAddress.city??registrant.city;registrant.state=verified.normalizedAddress.region??registrant.state;registrant.zip=verified.normalizedAddress.postalCode??registrant.zip;registrant.country=verified.normalizedAddress.countryCode??registrant.country;}
 let quote:Awaited<ReturnType<typeof getVercelDomainQuote>>;
