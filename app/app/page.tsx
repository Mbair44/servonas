...
export default async function AppHome(){
  ...
  return <main className="app-home"><section className="app-home-card"><div className="app-home-head"><div><span className="sv-kicker">Servonas</span><h1>{isPlatformAdmin?"All business workspaces":"Your workspaces"}</h1><p>Signed in as {user.email}</p></div><div className="crm-header-actions">{isPlatformAdmin&&<><Link className="sv-button" href="/app/admin/usage">Usage &amp; spend</Link><Link className="sv-button" href="/app/admin/twilio">Twilio activation</Link><Link className="sv-button" href="/app/admin/trials">Manage trials</Link><Link className="sv-button sv-secondary" href="/app/admin/entitlements">Manage access</Link></>}<form action={signOut}><button className="sv-button sv-secondary">Log out</button></form></div></div>{workspaces.length?<div className="workspace-list">{workspaces.map((w:any)=><Link key={w.id} href={`/app/${w.slug}`}><strong>{w.name}</strong><span>{w.role.replaceAll("_"," ")}</span></Link>)}</div>:<div className="sv-empty"><h2>Create your first business</h2><p>Your account is ready. Continue through onboarding to create a workspace and become its owner.</p><Link className="sv-button" href="/onboarding">Create your business</Link></div>}</section></main>
}
