...
export default async function NewJob({ params, searchParams }: { params: Promise<{ businessSlug: string }>; searchParams: Promise<{ customerId?: string }> }) {
  ...
  return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content">
    ...
    <section className="workspace-panel"><JobForm action={createJob.bind(null, businessSlug)} customers={customers ?? []} locations={locations ?? []} services={services ?? []} technicians={technicians ?? []} priorJobs={priorJobs??[]} submitLabel="Create job" defaultCustomerId={query.customerId} defaultStartAt={formatBusinessLocalInput(new Date().toISOString(),business.timezone)}/></section>
  </section></main>;
}
