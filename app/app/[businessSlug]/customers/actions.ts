...
export async function createCustomer(
  ...
  const { data, error } = await supabase.from("customers").insert({
    ...
    tax_exempt: text(formData, "taxExempt") === "true",
    tax_exemption_reference: text(formData, "taxExemptionReference") || null,
...
export async function createServicePlan(slug:string,customerId:string,formData:FormData){
 ...
 if(!Number.isInteger(intervalValue)||intervalValue<1||intervalValue>120||!["day","week","month","month_weekday","year"].includes(intervalUnit))redirect(`${target}?error=${encodeURIComponent("Choose a valid recurring cadence.")}`);
 ...
 const payload={
  ...
  auto_dispatch:formData.get("autoDispatch")==="on",
...
export async function updateServicePlan(slug:string,customerId:string,planId:string,formData:FormData){
 ...
 if(!name||!locationId||!serviceId||!startDate||!firstDate||!Number.isInteger(intervalValue)||intervalValue<1||intervalValue>120||!["day","week","month","month_weekday","year"].includes(intervalUnit)||!Number.isInteger(duration)||duration<1||duration>10080||!Number.isFinite(price)||price<0||!Number.isInteger(schedulingFlexDays)||schedulingFlexDays<0||schedulingFlexDays>30||Boolean(endDate&&endDate<startDate))redirect(`${target}?error=${encodeURIComponent("Review the service-plan dates, cadence, duration, price, and scheduling window.")}`);
