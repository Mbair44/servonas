...
export default async function NewCustomer({ params }: { params: Promise<{ businessSlug: string }> }) {
  ...
  if (!canManageCustomers(role)) return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile}/><section className="epic3-content"><div className="workspace-notice error">You do not have permission to add customers.</div></section></main>;
