...
export async function saveBookingSettings(slug:string,formData:FormData){
 ...
 if(formData.has("additionalDayPricingType")){
  ...
  const {error}=await supabase.from("booking_settings").update({standard_rental_hours:standardHours,allow_multi_day_rentals:checked(formData,"allowMultiDayRentals"),additional_day_pricing_type:pricingType,additional_day_discount_percent:discountPercent,additional_day_flat_rate_cents:flatRateRaw?Math.round(Number(flatRateRaw)*100):null,max_rental_days:maxDaysRaw?Number(maxDaysRaw):null,allow_extended_rental:checked(formData,"allowExtendedRental"),additional_hour_price_cents:Math.round(Math.max(0,number(formData,"additionalHourPrice"))*100),overnight_available:checked(formData,"overnightAvailable"),overnight_price_cents:Math.round(Math.max(0,number(formData,"overnightPrice"))*100),allow_same_day_multiple_bookings:checked(formData,"allowSameDayMultipleBookings"),updated_at:new Date().toISOString(),updated_by:user.id}).eq("business_id",business.id);
