...
export async function saveBookingSettings(slug:string,formData:FormData){
 ...
 const rentalDuration=number(formData,"rentalDurationMinutes",240);if(business.industry_profile==="party_rental"&&(!Number.isInteger(rentalDuration)||rentalDuration<30||rentalDuration>1440||rentalDuration%30!==0))redirect(`/app/${slug}/booking?error=Rental+length+must+use+30-minute+increments`);
