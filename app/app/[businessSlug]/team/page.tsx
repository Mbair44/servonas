...
export default async function TeamPage({params,searchParams}:{params:Promise<{businessSlug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const [{data:employees,error},{data:roles},{data:workIntervals},{data:exceptions},{data:jobs},{data:technicians},{data:qualifications},{data:capacityProfiles},{data:features},{count:pendingInvitations},{data:invitations},{data:importInvitationRows},{count:importIssues}]=await Promise.all([
  supabase.from("employees").select("id,preferred_name,legal_name,email,phone,employee_number,profile_photo_url,hire_date,termination_date,is_active,auth_user_id,created_at,employee_role_assignments!employee_roles_employee_tenant_fk(is_active,workforce_roles!employee_roles_role_tenant_fk(id,name))").eq("business_id",business.id).order("preferred_name"),
 ...
 const search=(q.search??"").trim().toLowerCase(),status=q.status??"active",roleFilter=q.role??"",emailFilter=q.email??"";
 const visible=(employees??[]).filter(employee=>{
  ...
  return (!search||[employee.preferred_name,employee.legal_name,employee.email,employee.employee_number].some(value=>String(value??"").toLowerCase().includes(search)))
   ...
   &&(emailFilter!=="missing"||!employee.email)
 ...
 const setupSummary=teamSetupSummary(employees??[],business.owner_user_id,pendingInvitations??0,importIssues??0);
 const recentEmployees=[...(employees??[])].sort((a,b)=>new Date(b.created_at).getTime()-new Date(a.created_at).getTime()).slice(0,5).map(employee=>{
  const employeeRoles=(employee.employee_role_assignments??[]).filter((item:any)=>item.is_active).map((item:any)=>relation(item.workforce_roles)).filter(Boolean);
  return {id:employee.id,preferredName:employee.preferred_name,role:(employeeRoles[0] as {name?:string}|undefined)?.name??"Employee",status:(!employee.auth_user_id&&employee.email&&pendingEmails.has(employee.email.toLowerCase())?"Pending invitation":employee.is_active?"Active":"Inactive") as "Active"|"Inactive"|"Pending invitation"};
 });
 ...
 return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content workforce-page">
  <header className="epic3-header"><div><small>Team</small><h1>Add your team</h1><p>Get your employees into Servonas.</p></div>{canEdit&&<a className="sv-button sv-secondary" href="#invite-employee">Invite employee</a>}</header>
  ...
  <TeamSetupLanding businessSlug={businessSlug} summary={setupSummary} recentEmployees={recentEmployees} canEdit={canEdit}/>
  ...
  <section className="workspace-panel workforce-directory" id="employee-directory"><div className="panel-title"><div><span className="sv-kicker">Directory</span><h2>Employees</h2></div><span>{visible.length} shown</span></div>
   <form className="workforce-filters"><input type="hidden" name="email" value={emailFilter}/><label>Search<input name="search" defaultValue={q.search??""} placeholder="Name, email, or employee #"/></label><label>Status<select name="status" defaultValue={status}><option value="active">Active</option><option value="inactive">Inactive</option><option value="all">All</option></select></label><label>Role<select name="role" defaultValue={roleFilter}><option value="">All roles</option>{roles?.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="sv-button sv-secondary">Filter</button>{emailFilter==="missing"&&<Link className="text-button" href={`/app/${businessSlug}/team#employee-directory`}>Clear missing email filter</Link>}</form>
