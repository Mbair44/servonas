...
export default async function LocalSeoPage({
  ...
  if(searchConsoleConnection?.status==="connected"&&searchConsoleConnection.property_url)await syncSearchConsole({businessId:business.id});
  const {data:searchConsoleRows}=searchConsoleConnection?.property_url?await supabase.from("business_google_search_console_rows").select("page_url,query,clicks,impressions,ctr,position,synced_at").eq("business_id",business.id).eq("property_url",searchConsoleConnection.property_url):{data:[]};
  ...
  const locationCards=[...locations.map(location=>({location,page:locationPageByKey.get(location.id.toLowerCase())??null,recommendation:locationRecommendationByKey.get(location.id)??null})),...report.recommendations.filter(item=>item.type==="missing_location_page"&&!locations.some(location=>location.id.toLowerCase()===item.entityId.toLowerCase())).map(item=>({location:{id:item.entityId,name:item.entityLabel,jobCount90d:0,customerCount:0,reviewCount:0},page:locationPageByKey.get(item.entityId.toLowerCase())??null,recommendation:item})),...(locationPages??[]).filter(page=>!locations.some(location=>location.id.toLowerCase()===String(page.source_location_key).toLowerCase())&&!report.recommendations.some(item=>item.type==="missing_location_page"&&item.entityId.toLowerCase()===String(page.source_location_key).toLowerCase())).map(page=>({location:{id:page.source_location_key,name:[page.city,page.state].filter(Boolean).join(", "),jobCount90d:0,customerCount:0,reviewCount:0},page,recommendation:null}))];
  ...
  return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile} /><section className="epic3-content marketing-page local-seo-page">
    ...
    <section className="workspace-panel">
      ...
      {searchConsoleConnection?.status==="connected"&&searchConsoleConnection.property_url?<p><strong>Connected</strong> · {searchConsoleConnection.property_url}{searchConsoleConnection.last_synced_at?` · Last synced ${compactDate(searchConsoleConnection.last_synced_at)}`:" · Waiting for first sync"} · <Link href={`/api/google-search-console/connect/${encodeURIComponent(businessSlug)}`}>Reconnect or change property</Link></p>:searchConsoleConnection?.status==="property_selection_required"?<form action={selectSearchConsoleProperty.bind(null,businessSlug)}><p>Choose the verified property that represents your public website.</p><select name="property" required>{(Array.isArray(searchConsoleConnection.available_properties)?searchConsoleConnection.available_properties:[]).map((entry:any)=><option key={entry.siteUrl} value={entry.siteUrl}>{entry.siteUrl}{propertyMatchesDomain(String(entry.siteUrl),websiteBase)?" — recommended":""}</option>)}</select><button className="sv-button">Use this property</button></form>:<><p>{searchConsoleConnection?.status==="permission_denied"?"Google denied access. Reconnect with access to a verified property.":searchConsoleConnection?.last_error_message||"Connect Search Console to see organic impressions, clicks, queries, and location-page opportunities."}</p><Link className="sv-button" href={`/api/google-search-console/connect/${encodeURIComponent(businessSlug)}`}>Connect Search Console</Link></>}
    </section>
    ...
    <section className="workspace-panel local-seo-location-opportunities">
      ...
      <div className="local-seo-card-list">{locationCards.length?locationCards.map(({location,page,recommendation})=>{
        ...
        if(page?.status==="published"&&pageUrl){
          ...
          const organicRows=(searchConsoleRows??[]).filter((row:any)=>String(row.page_url).replace(/\/$/,"")===normalizedPageUrl);
          ...
          return <article className="local-seo-card location-opportunity-card local-growth-card priority-healthy" id={`location-${city.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`} key={location.id}>
            ...
            {organicRows.length?<div className="local-seo-health-list"><strong>Google Search — Last 28 days</strong><article><span>Impressions</span><span>{organic.impressions}</span></article><article><span>Clicks</span><span>{organic.clicks}</span></article><article><span>CTR</span><span>{organic.impressions?`${Math.round(organic.clicks/organic.impressions*100)}%`:"—"}</span></article><article><span>Avg. position</span><span>{organic.impressions?(organic.positionTotal/organic.impressions).toFixed(1):"—"}</span></article>{topQueries.length?<article><span>Top queries</span><span>{topQueries.map((row:any)=>row.query).filter(Boolean).join(" · ")}</span></article>:null}</div>:null}
        ...
        return <article className={`local-seo-card location-opportunity-card ${recommendation?.priority==="high"?"priority-high":"priority-medium"}`} key={location.id}><span className="local-seo-badge">{page?.status==="draft"?"Draft ready":"Ready to build"}</span><h3>{page?.status==="draft"?`Finish your ${city} page`:`${location.name} is a strong location-page opportunity`}</h3><p>{page?.status==="draft"?"Servonas created the page from your business information. Review it, then publish when ready.":`People nearby search for services in ${city}. A dedicated ${city} page can help Google understand that you serve this area.`}</p><div className="location-opportunity-status"><span>{page?"Draft page found":`No ${city} page found`}</span><span>{serviceAreaSupported?`${city} is already in your service area`:location.jobCount90d||location.customerCount?"Existing customer activity confirms this area":"Review service coverage before publishing"}</span><span>{allServices.length?`${allServices.length} business services/products available`:"Add services or products before building"}</span></div><div className="local-seo-actions">{!page&&recommendation?<form action={buildLocationPage.bind(null,businessSlug,location.id,recommendation.dedupeKey)}><LocationPageSubmit label={`Build ${city} Page`} pendingLabel={`Building ${city} page…`}/></form>:page?.status==="draft"?<Link className="sv-button" href={`/app/${businessSlug}/marketing/seo/locations/${page.id}`}>Finish {city} Page</Link>:null}<details className="location-opportunity-why"><summary>Why this helps</summary><p>A useful city page gives customers one place to see what you offer in their area and gives Google clearer information about where your business works.</p></details></div></article>;
