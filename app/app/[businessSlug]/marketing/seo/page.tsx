    ...
    <section className="workspace-panel local-seo-location-opportunities">
      ...
      <div className="local-seo-card-list">{locationCards.length?locationCards.map(({location,page,recommendation})=>{
        ...
        if(page?.status==="published"&&pageUrl){
          ...
          const growth=buildLocalGrowthPlan({businessSlug,city,pageUrl:normalizedPageUrl,pageLive:website?.status==="published",metadataReady:Boolean(page.page_title&&page.meta_description),sitemapReady:website?.status==="published",internallyLinked:serviceAreaSupported,serviceAreaSupported,googleBusinessConnected:["connected","account_discovery_rate_limited"].includes(googleConnection?.status??""),googleAdsConnected:googleAdsConnection?.status==="connected"&&Boolean(googleAdsConnection.google_ads_customer_id),locationPromotionExists:(googleAdGroups??[]).some(group=>group.status==="published"&&String(group.destination_url??"").replace(/\/$/,"")===normalizedPageUrl),trackingReady:website?.status==="published"});
          return <article className="local-seo-card location-opportunity-card local-growth-card priority-healthy" key={location.id}>
