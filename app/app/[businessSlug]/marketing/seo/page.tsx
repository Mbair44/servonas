...
export default async function LocalSeoPage({
  ...
  return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile} /><section className="epic3-content marketing-page local-seo-page">
    ...
    <section className="workspace-panel">
      ...
      {searchConsoleState?.status==="connected"&&searchConsoleState.property_url?<p><strong>Connected</strong> · {searchConsoleState.property_url}{searchConsoleState.last_synced_at?` · Last synced ${compactDate(searchConsoleState.last_synced_at)}`:" · Waiting for first sync"} · <a href={`/api/google-search-console/connect/${encodeURIComponent(businessSlug)}`}>Reconnect or change property</a></p>:searchConsoleState?.status==="property_selection_required"?<form action={selectSearchConsoleProperty.bind(null,businessSlug)}><p>Choose the verified property that represents your public website.</p><select name="property" required>{(Array.isArray(searchConsoleState.available_properties)?searchConsoleState.available_properties:[]).map((entry:any)=><option key={entry.siteUrl} value={entry.siteUrl}>{entry.siteUrl}{propertyMatchesDomain(String(entry.siteUrl),websiteBase)?" — recommended":""}</option>)}</select><button className="sv-button">Use this property</button></form>:<><p>{searchConsoleState?.status==="permission_denied"?"Google denied access. Reconnect with access to a verified property.":searchConsoleState?.last_error_message||searchConsoleResult.error?.message||"Connect Search Console to see organic impressions, clicks, queries, and location-page opportunities."}</p><a className="sv-button" href={`/api/google-search-console/connect/${encodeURIComponent(businessSlug)}`}>Connect Search Console</a></>}
    </section>
