...
export async function selectSearchConsoleProperty(businessSlug:string,formData:FormData){const property=text(formData.get("property"),500);const {supabase,business,role}=await requireWorkspace(businessSlug);if(!canManageBusiness(role)||!property)redirect(destination(businessSlug,"error","Choose a Search Console property."));const {data:connection}=await supabase.from("business_google_search_console_connections").select("available_properties").eq("business_id",business.id).maybeSingle();const allowed=Array.isArray(connection?.available_properties)&&connection.available_properties.some((entry:any)=>entry?.siteUrl===property);if(!allowed)redirect(destination(businessSlug,"error","That Search Console property is not available to this account."));const {error}=await supabase.from("business_google_search_console_connections").update({property_url:property,status:"connected",last_error_code:null,last_error_message:null,updated_at:new Date().toISOString()}).eq("business_id",business.id);if(error)redirect(destination(businessSlug,"error","Search Console property could not be saved."));revalidatePath(pagePath(businessSlug));redirect(destination(businessSlug,"success","Search Console property selected. Reporting will sync shortly."));}
