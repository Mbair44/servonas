  ...
  {hasCampaigns && <section className="google-ads-primary-stack">
   <section className="google-ads-campaign-grid">
    {campaignCards.map(({ campaign, metric, effectiveGoogleStatus, effectivePrimaryStatus, primaryStatusReasons, statusSyncUnavailable, issuesAvailable, effectiveCardStatus, statusLabel, summary }) => {
     ...
     const liveCampaignBiddingStrategy = campaign.google_campaign_id ? campaignHealthSnapshotsByCampaignId.get(String(campaign.google_campaign_id))?.biddingStrategyType ?? null : null;
     const effectiveCampaignBiddingStrategy = liveCampaignBiddingStrategy ?? campaign.bidding_strategy ?? "MAXIMIZE_CLICKS";
     ...
     <section className="google-ads-ad-group-panel" aria-label="Ad groups">
      ...
      <div className="google-ads-ad-group-list">
       {draftAdGroups.map((adGroup: any) => {
        ...
        const effectiveBiddingStrategy=matchedLive?.biddingStrategyType??effectiveCampaignBiddingStrategy;
        const supportsAdGroupCpc=effectiveBiddingStrategy==="MANUAL_CPC";
        ...
        return <details key={adGroup.id} className="google-ads-ad-group-card">
         ...
         <div className="google-ads-ad-group-body">
          ...
          {supportsAdGroupCpc?<section className="google-ads-ad-group-cpc"><div><span>Max CPC</span><strong>{currentCpcMicros>0?microsToMoney(currentCpcMicros):"Not set"}</strong><small>Servonas suggestion: <b>{microsToMoney(recommendedCpcMicros)}</b>. {cpcRecommendationReason}</small></div>{publishedInGoogle?<form action={updateGoogleAdsAdGroupCpcAction.bind(null,businessSlug,campaign.id,adGroup.id)}><label><span className="sr-only">Maximum CPC in dollars</span><span className="google-ads-input-with-unit"><span>$</span><input name="maxCpcDollars" type="number" min="0.01" max="25" step="0.01" required defaultValue={currentCpcMicros>0?(currentCpcMicros/1_000_000).toFixed(2):recommendedCpcDollars}/></span></label><button className="sv-button" data-loading-label="Updating Google Ads…">Update Max CPC</button></form>:<small>The recommended amount will be used when this ad group is created unless you change it below.</small>}</section>:<section className="google-ads-ad-group-cpc automated"><div><span>Stored default CPC</span><strong>{currentCpcMicros>0?microsToMoney(currentCpcMicros):"Not set"}</strong><small>Google reports {effectiveBiddingStrategy==="MAXIMIZE_CLICKS"?"Maximize Clicks":"automated bidding"}, so this stored amount is currently ignored. Switch the campaign to Manual CPC before editing individual ad-group bids.</small></div></section>}
