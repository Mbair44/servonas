    ...
    {campaignCards.map(({ campaign, metric, effectiveGoogleStatus, effectivePrimaryStatus, primaryStatusReasons, statusSyncUnavailable, issuesAvailable, effectiveCardStatus, statusLabel, summary }) => {
     ...
     <section className="google-ads-performance-block" aria-label="Campaign performance">
      ...
      <dl className="google-ads-facts google-ads-performance-facts">
       ...
       <div><dt>CTR <span className="google-ads-metric-help" tabIndex={0} role="img" aria-label="Click-through rate help" data-tooltip="Click-through rate: the percentage of ad impressions that turned into clicks. Higher can mean the ad is relevant to the search.">?</span></dt><dd>{metric ? `${metric.ctr.toFixed(1)}%` : "—"}</dd></div>
          ...
          <div className="google-ads-health-list">
           <section>
            ...
            <article>
              ...
              <span>{recommendedKeywordReviewAction(recommendation.category, relevantKeywords)}</span>
             </article>
            ...
            {automatedBidding ? <article><strong>Manual review required</strong><span>Google is managing bids automatically for this campaign, so manual bid changes are not available.</span></article> : null}
            {recommendation.category === "bid" && !bidRecommendation && !bidCandidates.length && !defaultBidAction && !automatedBidding ? <article><strong>Manual review required</strong><span>Servonas could not verify a current Manual CPC default bid and target ad group, so it cannot safely apply this change.</span></article> : null}
            <article>
             <strong>Apply status</strong>
             <span>{bidRecommendation || defaultBidAction || (bidCandidates.length && manualCpc) ? "Action available. Review the editable bid change above before confirming." : "Manual review required."}</span>
            </article>
           </section>
          </div>
         </details>; })}
