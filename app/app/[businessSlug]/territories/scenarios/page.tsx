...
import {compareSimulations,simulateTerritories,type SimulationLocation,type SimulationComparisonMetric} from "@/lib/territorySimulation";
...
export default async function ScenarioPlannerPage({params,searchParams}:{params:Promise<{businessSlug:string}>;searchParams:Promise<{scenario?:string;success?:string;error?:string}>}){
 ...
 const currentSimulation=simulateTerritories(liveTerritories??[],liveTerritories??[],simulationLocations);
 const simulation=simulateTerritories(liveTerritories??[],proposed,simulationLocations);
 const simulationComparison=compareSimulations(currentSimulation,simulation);
 const comparison=summarizeScenario(liveTerritories?.length??0,scenarioTerritories??[]);
 const money=new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0});
 const formatComparison=(metric:SimulationComparisonMetric,value:number|null)=>{
  if(value===null)return "Not measured";
  if(metric.unit==="currency")return money.format(value);
  if(metric.unit==="miles")return `${value.toFixed(1)} mi`;
 ...
 return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content scenario-planner">
  ...
  {error?<div className="workspace-notice error">{error.code==="42703"?"Apply the Epic 8.5 Checkpoint 11 live simulation migration.":"Scenario planning could not be loaded. Review the server logs."}</div>:<div className="scenario-workspace">
   ...
   <section className="scenario-detail">{selected?<><header><div><span>{selected.status} scenario · simulation revision {selected.simulation_revision}</span><h2>{selected.name}</h2><p>{selected.description||"No planning notes yet."}</p></div><strong>Version {selected.version}</strong></header>
    ...
    <section className="scenario-before-after"><header><div><span className="sv-kicker">Before / after</span><h3>Current compared with proposed</h3></div><small>Positive labels indicate the preferred direction</small></header><div className="scenario-comparison-table" role="table" aria-label="Current and proposed territory comparison"><div className="head" role="row"><strong>Measure</strong><strong>Current</strong><strong>Proposed</strong><strong>Difference</strong></div>{simulationComparison.map(metric=>{const preferred=metric.difference!==null&&metric.difference!==0&&(metric.lowerIsBetter?metric.difference<0:metric.difference>0);return <div role="row" key={metric.key}><span>{metric.label}</span><span>{formatComparison(metric,metric.current)}</span><span>{formatComparison(metric,metric.proposed)}</span><strong className={preferred?"improved":metric.difference?"declined":""}>{metric.difference===null?"Not measured":`${metric.difference>0?"+":""}${formatComparison(metric,metric.difference)}`}</strong></div>})}</div><footer>Revenue per technician, jobs per technician, route balance, and hiring capacity remain unscored until employee capacity is an authoritative input.</footer></section>
