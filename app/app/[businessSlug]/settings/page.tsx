...
import {manageServonasSubscription,refreshServonasSubscription,startServonasSubscription} from "@/app/onboarding/actions";
...
export default async function Settings({params,searchParams}:{params:Promise<{businessSlug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const [{data:paymentAccount,error:paymentAccountError},{data:endpointDefaults},{data:technicians},{data:endpointOverrides},{data:routingPolicy},{data:numbering},{data:platformSubscription},{data:invoicePaymentOptions},{data:inboundSmsSettings}]=await Promise.all([
  ...
  subscriptionBillingEnabled?supabase.from("business_platform_subscriptions").select("status,trial_ends_at,current_period_ends_at,stripe_customer_id").eq("business_id",business.id).maybeSingle():Promise.resolve({data:null}),
 ...
 return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content settings-page-redesign">
  ...
  {subscriptionBillingEnabled&&editable&&<section className="settings-summary-card" id="servonas-subscription"><header className="settings-section-header"><div><span>Servonas billing</span><h2>Software subscription</h2><p>This pays for your Servonas software access and is separate from Stripe Connect customer payments.</p></div></header><div className="settings-payment-summary"><div><b>{["active","trialing"].includes(platformSubscription?.status??"")?"Billing configured":platformSubscription?.status==="checkout_pending"?"Setup awaiting confirmation":"Payment method needed"}</b><span className={`estimate-status ${platformSubscription?.status==="active"?"paid":"draft"}`}>{platformSubscription?.status?.replaceAll("_"," ")??"not added"}</span></div><p>{platformSubscription?.trial_ends_at?`Your free trial ends ${formatTrialDate(platformSubscription.trial_ends_at,business.timezone)}.`:"Your first 30 days are free. Add billing before the trial deadline to prevent the workspace from being locked."}</p>{["active","trialing","past_due","paused"].includes(platformSubscription?.status??"")?<form action={manageServonasSubscription.bind(null,businessSlug)}><button className="sv-button">Update billing</button></form>:platformSubscription?.status==="checkout_pending"?<div className="settings-payment-actions"><form action={refreshServonasSubscription.bind(null,businessSlug)}><button className="sv-button">Check billing status</button></form><form action={startServonasSubscription.bind(null,businessSlug,"settings")}><button className="sv-button sv-secondary">Resume billing setup</button></form></div>:<form action={startServonasSubscription.bind(null,businessSlug,"settings")}><button className="sv-button">Add Servonas billing</button></form>}</div></section>}
 </section></main>;
}
