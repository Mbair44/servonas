...
import { connectStripe, disconnectStripe, refreshStripeStatus, updateBusinessSettings, updateEmployeeNumbering, updateInboundSmsSettings, updateInvoicePaymentOptions, updateMissedCallRecoverySettings, updateRouteEndpoints, updateRoutingPolicy } from "./actions";
...
export default async function Settings({params,searchParams}:{params:Promise<{businessSlug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const [{data:paymentAccount,error:paymentAccountError},{data:endpointDefaults},{data:technicians},{data:endpointOverrides},{data:routingPolicy},{data:numbering},{data:invoicePaymentOptions},{data:inboundSmsSettings},{data:missedCallSettings}]=await Promise.all([
 ...
 return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content settings-page-redesign">
  ...
  {editable&&<section className="settings-summary-card" id="missed-call-recovery"><header className="settings-section-header"><div><span>Lead recovery</span><h2>Missed Call Recovery</h2><p>Text missed callers immediately, qualify the request, escalate urgent issues, and book when a confirmed time is available.</p></div></header><form action={updateMissedCallRecoverySettings.bind(null,businessSlug)} className="settings-drawer-form single"><label className="settings-check wide"><input name="enabled" type="checkbox" defaultChecked={missedCallSettings?.enabled}/>Enable missed-call recovery</label><label className="wide">Twilio business number<input required name="recoveryNumber" type="tel" defaultValue={missedCallSettings?.recovery_number_e164??process.env.TWILIO_PHONE_NUMBER??""} placeholder="+14805550123"/></label><label className="wide">Initial missed-call text<textarea required name="initialSms" maxLength={1200} defaultValue={missedCallSettings?.initial_sms_body??"Sorry we missed your call. How can we help? Reply with your name and what service you need. Reply STOP to opt out."}/></label><label className="settings-check wide"><input name="aiEnabled" type="checkbox" defaultChecked={missedCallSettings?.ai_enabled??true}/>Use AI to continue the intake conversation</label><label className="wide">AI behavior<textarea required name="aiInstructions" maxLength={4000} defaultValue={missedCallSettings?.ai_instructions??"Be concise, helpful, and focused on collecting the customer name, service address, issue, urgency, and preferred appointment time. Never diagnose, promise pricing, or minimize an emergency."}/></label><label className="settings-check wide"><input name="bookingEnabled" type="checkbox" defaultChecked={missedCallSettings?.booking_enabled??true}/>Allow confirmed appointments when service availability is known</label><label className="wide">Emergency/on-call alert number<input name="alertPhone" type="tel" defaultValue={missedCallSettings?.alert_phone_e164??""} placeholder="+14805550123"/></label><p className="wide">Set the Twilio voice status callback to <code>{`${process.env.NEXT_PUBLIC_APP_URL??"https://your-domain.com"}/api/twilio/missed-call`}</code> for no-answer, busy, failed, and canceled calls. Incoming texts continue to use <code>{`${process.env.NEXT_PUBLIC_APP_URL??"https://your-domain.com"}/api/twilio/inbound`}</code>.</p><button className="sv-button">Save missed-call recovery</button></form></section>}
 </section></main>;
}
