...
export default async function Settings({params,searchParams}:{params:Promise<{businessSlug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content settings-page-redesign">
  ...
  {editable&&<MissedCallRecoverySettings settings={missedCallSettings} defaultPhone={process.env.TWILIO_PHONE_NUMBER??""} baseUrl={process.env.NEXT_PUBLIC_APP_URL??"https://your-domain.com"} action={updateMissedCallRecoverySettings.bind(null,businessSlug)}/>}
  {editable&&isPool&&<PoolServiceSettings settings={poolSettings} ranges={poolRanges??[]} chemicals={poolChemicals?.length?poolChemicals.map(item=>item.name):defaultPoolChemicals} checklist={poolChecklist?.length?poolChecklist.map(item=>item.label):defaultPoolChecklist} action={updatePoolServiceSettings.bind(null,businessSlug)}/>}
 </section></main>;
}
