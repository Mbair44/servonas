...
const normalizeMetaPixelId=(value:string)=>{
 const normalized=value.trim();
 if(!normalized)return null;
 return /^[0-9]{8,24}$/.test(normalized)?normalized:"invalid";
};
...
export async function saveWebsiteSettings(slug:string,data:FormData){
 ...
 const fullDayRentalSettings=business.industry_profile==="party_rental"&&data.has("fullDayRentalMessage")?{full_day_rental_message:text(data,"fullDayRentalMessage")}:{};
 if((fullDayRentalSettings.full_day_rental_message?.length??0)>1000)redirect(target(slug,"error","Keep the full-day rental message within 1,000 characters."));
 const cancellationSettings=data.get("cancellationPolicySettingsPresent")==="true"?{cancellation_policy_enabled:data.get("cancellationPolicyEnabled")==="on",cancellation_policy_text:text(data,"cancellationPolicyText"),require_cancellation_acknowledgment:data.get("requireCancellationAcknowledgment")==="on"}:{};
 if("cancellation_policy_text" in cancellationSettings&&(cancellationSettings.cancellation_policy_text!.length>10000||(cancellationSettings.cancellation_policy_enabled&&!cancellationSettings.cancellation_policy_text)))redirect(target(slug,"error","Enter cancellation policy text (up to 10,000 characters)."));
 ...
 const metaPixelId=normalizeMetaPixelId(text(data,"metaPixelId"));
 if(metaPixelId==="invalid")redirect(target(slug,"error","Use a valid numeric Meta Pixel ID, like 2375527282981645."));
