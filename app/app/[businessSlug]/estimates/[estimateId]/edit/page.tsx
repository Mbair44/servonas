...
export default async function EditEstimate({ params }: { params: Promise<{ businessSlug:string; estimateId:string }> }) {
  ...
  if(!canManageCustomers(role)||estimate.status!=="draft") return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile}/><section className="epic3-content"><div className="workspace-notice error">This estimate is not editable. Start a revision first.</div></section></main>;
