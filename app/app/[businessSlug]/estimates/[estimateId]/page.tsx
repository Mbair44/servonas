...
import { convertEstimateToInvoice, convertEstimateToJob, duplicateEstimate, resendEstimateEmail, reviseEstimate, sendEstimate, voidEstimate } from "../actions";
...
export default async function EstimateDetail({ params, searchParams }: { params:Promise<{businessSlug:string;estimateId:string}>; searchParams:Promise<{success?:string;error?:string;publicLink?:string}> }) {
  ...
  const [{data:estimate},{data:lines},{data:fees},{data:events},{data:delivery}] = await Promise.all([
    ...
    supabase.from("estimate_communication_events").select("status").eq("estimate_id",estimateId).eq("business_id",business.id).eq("event_type","estimate_sent").order("created_at",{ascending:false}).limit(1).maybeSingle(),
  ]);
  ...
  return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name}/><section className="epic3-content">
    ...
    {canEdit&&<section className="estimate-actions workspace-panel">
      ...
      {["sent","viewed"].includes(estimate.status)&&delivery?.status!=="sent"&&<form action={resendEstimateEmail.bind(null,businessSlug,estimateId)}><button className="sv-button">Retry email</button></form>}
      ...
      {["accepted","converted"].includes(estimate.status)&&<form action={convertEstimateToInvoice.bind(null,businessSlug,estimateId)}><button className="sv-button sv-secondary">Convert to invoice</button></form>}
