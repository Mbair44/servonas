...
import RouteCalculationSubmit from "@/components/RouteCalculationSubmit";
...
export default async function DispatchPage({ params, searchParams }: { params: Promise<{ businessSlug: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile}/><section className="epic3-content dispatch-page">
    ...
    <section className="workspace-panel dispatch-toolbar"><div><Link aria-label="Previous day" href={hrefFor(addDays(date, -1))}>‹</Link><Link className="sv-button sv-secondary" href={hrefFor(today)}>Today</Link><Link aria-label="Next day" href={hrefFor(addDays(date, 1))}>›</Link></div><form><label>Date<input key={date} name="date" type="date" defaultValue={date}/></label><button className="sv-button">Go</button></form><strong>{new Intl.DateTimeFormat("en-US", { timeZone: "UTC", weekday: "long", month: "long", day: "numeric" }).format(new Date(`${date}T12:00:00Z`))}</strong>{canEdit&&<><form action={calculateDispatchRoutes.bind(null,businessSlug)}><input type="hidden" name="date" value={date}/><RouteCalculationSubmit recalculate={routePlan?.calculation_status==="ready"}/></form>{routePlan?.calculation_status==="ready"&&<form action={optimizeDispatchRoutes.bind(null,businessSlug)}><input type="hidden" name="date" value={date}/><input type="hidden" name="routePlanId" value={routePlan.id}/><input type="hidden" name="planVersion" value={routePlan.version}/><button className="sv-button sv-secondary" type="submit">Optimize routes</button></form>}</>}</section>
