...
export default async function NewPriceBookItem({ params }: { params: Promise<{ businessSlug: string }> }) {
  ...
  if (!canManageCustomers(role)) return <main className="epic3-shell"><WorkspaceNav slug={businessSlug} name={business.name} industry={business.industry_profile}/><section className="epic3-content"><div className="workspace-notice error">You do not have permission to manage the price book.</div></section></main>;
