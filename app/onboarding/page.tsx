...
export default async function Onboarding({searchParams}:{searchParams:Promise<{business?:string;saved?:string}>}){
 ...
 if(query.business){
  const {data:membership}=await s.from("business_members").select("role,businesses!inner(id,name,display_name,slug,operating_model,industry_profile)").eq("user_id",user.id).eq("businesses.slug",query.business).maybeSingle();
