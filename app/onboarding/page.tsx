...
export default async function Onboarding({searchParams}:{searchParams:Promise<{business?:string;saved?:string;error?:string;billing?:string;billingAdded?:string;source?:string;websiteStep?:string;celebrate?:string;celebrationAt?:string}>}){
 ...
 if(query.business){
  ...
  const {data:websiteFirst}=await s.from("business_website_onboarding_states").select("source,current_step,preview_reached_at").eq("business_id",business.id).maybeSingle(),websiteConfig=getWebsiteFirstConfig(websiteFirst?.source);
  if(websiteFirst&&websiteConfig&&websiteFirst.current_step!=="completed"){const step=query.websiteStep||websiteFirst.current_step;return <main className="onboarding-shell website-first-onboarding">{step==="preview"?<WebsiteFirstPreview businessId={business.id} businessSlug={business.slug} source={websiteConfig.source} celebrate={query.celebrate==="1"} celebrationAt={query.celebrationAt??websiteFirst.preview_reached_at??undefined}/>:<WebsiteFirstStyle businessSlug={business.slug} error={query.error} source={websiteConfig.source}/>}</main>;}
