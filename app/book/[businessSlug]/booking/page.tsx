...
export async function generateMetadata({params}:{params:Promise<{businessSlug:string}>}):Promise<Metadata>{
  ...
  if(!data)return {};
  return {title:`Complete Reservation | ${data.businessName??"Business"}`,robots:{index:false,follow:true},icons:data.bookingLogo?{icon:[{url:data.bookingLogo}],shortcut:data.bookingLogo,apple:data.bookingLogo}:undefined};
}
