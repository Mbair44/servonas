...
function signupHref(params:Record<string,string|string[]|undefined>){const query=new URLSearchParams();for(const key of attributionKeys){const raw=params[key],value=Array.isArray(raw)?raw[0]:raw;if(value)query.set(key,value.slice(0,500));}const suffix=query.toString();return `/signup${suffix?`?${suffix}`:""}`;}
...
export default async function PestControlWebsitePage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 const signup=signupHref(await searchParams);
 return <main className="pcw-page">
  <section className="pcw-hero"><div className="pcw-orb pcw-orb-one"/><div className="pcw-orb pcw-orb-two"/><div className="sv-container pcw-hero-grid">
   <div className="pcw-hero-copy"><span className="sv-kicker">Websites for pest control companies</span><h1>We&apos;ll Build Your Pest Control Website. <span>Free.</span></h1><p>Get a professional website built for your pest control business, plus the tools to manage bookings, customers, scheduling, invoices, payments, and more.</p><div className="sv-actions"><Link className="sv-button pcw-primary" href={signup}>Build My Free Website <span aria-hidden="true">→</span></Link><a className="sv-button sv-secondary" href="#included">See What&apos;s Included</a></div><div className="pcw-hero-proof"><span>✓ Built for small pest businesses</span><span>✓ Mobile friendly</span><span>✓ No existing website needed</span></div></div>
