...
Phase 2.5 implements `SubaccountWebhookSecretResolver` with Supabase Vault. Tenant validation resolves `AccountSid + To`, retrieves the token using both business and subaccount identity, and never falls back to the parent token. Existing parent-account SMS validation and behavior remain intact. See `twilio-multi-tenant-phase-2-5.md`.

Before Phase 3: finish/approve the Servonas primary ISV profile and confirm Twilio's policy field requirements. Phase 3 can add Messaging Services and A2P registration/association.
