...
export async function sendInvoiceFinancialEmail(invoiceId:string,event:FinancialEmailEvent,options:{paymentId?:string;refundId?:string;publicUrl?:string}={}){
 ...
 const live=process.env.EMAIL_DELIVERY_MODE==="live",payload={business_id:i.business_id,invoice_id:invoiceId,payment_id:options.paymentId??null,refund_id:options.refundId??null,event_type:event,recipient_email:recipient,status:live?"queued":"stubbed"};
