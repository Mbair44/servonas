...
export type FinancialEmailEvent="invoice_sent"|"payment_link_sent"|"payment_succeeded"|"payment_failed"|"partial_payment"|"invoice_paid"|"invoice_overdue"|"refund_issued"|"receipt_sent";
...
export async function sendInvoiceFinancialEmail(invoiceId:string,event:FinancialEmailEvent,options:{paymentId?:string;refundId?:string;publicUrl?:string}={}){
 ...
 const {data:i,error}=await db.from("invoices").select("id,business_id,customer_id,invoice_number,currency,balance_due_cents,businesses(name),customers!invoices_customer_fk(first_name,last_name,email)").eq("id",invoiceId).maybeSingle();
 ...
 const {data:billingProfile}=customer?await db.from("customer_billing_profiles").select("billing_email").eq("business_id",i.business_id).eq("customer_id",i.customer_id).maybeSingle():{data:null};
 const recipient=billingProfile?.billing_email||customer?.email;
 ...
 const labels:Record<FinancialEmailEvent,string>={invoice_sent:"Invoice sent",payment_link_sent:"Payment link",payment_succeeded:"Payment received",payment_failed:"Payment failed",partial_payment:"Partial payment received",invoice_paid:"Invoice paid",invoice_overdue:"Invoice overdue",refund_issued:"Refund issued",receipt_sent:"Payment receipt"};
 ...
 const payload={business_id:i.business_id,invoice_id:invoiceId,payment_id:options.paymentId??null,refund_id:options.refundId??null,event_type:event,recipient_email:recipient,status:key&&from?"queued":"failed",error_message:key&&from?null:"Email delivery is not configured."};
 ...
 if(!key||!from){console.error("Financial email delivery is not configured",{invoiceId,event,missing:[!key?"RESEND_API_KEY":null,!from?"EMAIL_FROM":null].filter(Boolean)});return {ok:false};}
 try{const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:[recipient],subject:`${labels[event]} — ${business?.name||"Servonas"} ${i.invoice_number}`,text:lines.join("\n\n"),html:`<div style="font-family:Arial,sans-serif;line-height:1.6">${lines.map(x=>`<p>${esc(x)}</p>`).join("")}</div>`})});const result=await response.json() as {id?:string;message?:string};if(!response.ok||!result.id)throw new Error(result.message||`Resend HTTP ${response.status}`);await db.from("financial_notification_events").update({status:"sent",provider_message_id:result.id,sent_at:new Date().toISOString(),error_message:null}).eq("id",saved.data.id);console.info("Financial email delivered",{invoiceId,event,recipient,messageId:result.id});return {ok:true};}catch(e){const message=e instanceof Error?e.message:"Email failed";await db.from("financial_notification_events").update({status:"failed",error_message:message.slice(0,1000)}).eq("id",saved.data.id);console.error("Financial email delivery failed",{invoiceId,event,message});return {ok:false};}
}
