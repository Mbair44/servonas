...
export type MechanicalBullLandingData={
 businessId:string;businessName:string;businessSlug:string;phone:string|null;brandColor:string;bookingSlug:string;serviceAreas:string[];
...
export async function loadMechanicalBullLandingData(db:SupabaseClient,businessId?:string):Promise<MechanicalBullLandingData|null>{
 let businessQuery=db.from("businesses").select("id,name,slug,phone,primary_color,city,state,industry_profile").eq("is_deleted",false).eq("industry_profile","party_rental");
 ...
 const [{data:settings},{data:items},{data:territories}]=await Promise.all([
