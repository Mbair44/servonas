...
const accessTokenForPixel=(pixelId:string)=>{
 ...
 return token&&(!configuredPixel||configuredPixel===pixelId)?token:null;
};
...
export async function sendMetaConversion(input:MetaConversionInput){
 ...
 try{
  const response=await fetch(`https://graph.facebook.com/${graphVersion}/${input.pixelId}/events`,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify(body),cache:"no-store"});
  const result=await response.json().catch(()=>({})) as {events_received?:number;fbtrace_id?:string;error?:{message?:string;type?:string;code?:number;error_subcode?:number}};
  if(!response.ok||result.error||Number(result.events_received??0)<1)throw Object.assign(new Error(result.error?.message||`Meta CAPI returned HTTP ${response.status}`),{status:response.status,code:result.error?.code,subcode:result.error?.error_subcode,type:result.error?.type,traceId:result.fbtrace_id});
  ...
  const details=error as Error&{status?:number;code?:number;subcode?:number;type?:string;traceId?:string};
  ...
  console.error("Meta CAPI event failed",{stage:"event_send",businessId:input.businessId,businessSlug:input.businessSlug,pixelId:input.pixelId,event:input.event,event_id:input.eventId,httpStatus:details.status??null,errorCode:details.code??null,errorSubcode:details.subcode??null,errorType:details.type??null,providerTraceId:details.traceId??null,message:details.message});
