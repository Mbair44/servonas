...
export async function applyAssistedBookingAttribution(db:SupabaseClient,input:{businessId:string;bookingId:string;attribution:AssistedAttribution}){
 ...
 if(input.attribution.kind==="tracked"){
  ...
  const {error:snapshotError}=await db.from("booking_attribution_snapshots").upsert({...session,booking_id:input.bookingId,business_id:input.businessId,attribution_session_id:session.id,attribution_evidence:"tracked_session",updated_at:now},{onConflict:"booking_id"});if(snapshotError)throw snapshotError;
