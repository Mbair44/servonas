...
type RentalJobDatabaseError={code?:string;message?:string;details?:string;hint?:string};

export class RentalBookingJobPersistenceError extends Error{
 constructor(readonly operation:string,readonly resource:string,readonly databaseError:RentalJobDatabaseError|null|undefined){
  super(`${operation} failed (${databaseError?.code??"unknown"}).`);
  this.name="RentalBookingJobPersistenceError";
 }
}
...
export async function ensureRentalBookingJob(db:SupabaseClient,bookingId:string){
 ...
 const payload={business_id:booking.business_id,customer_id:booking.customer_id,service_location_id:location.id,title:items.length===1&&firstItem?.name?firstItem.name:"Party rental reservation",status:"scheduled",starts_at:start.toISOString(),ends_at:end.toISOString(),service_address:address,description:["Rental items:",...itemSummary,booking.notes?`Notes: ${booking.notes}`:""].filter(Boolean).join("\n"),subtotal:(Number(booking.subtotal_cents??booking.total_cents??0)+Number(booking.delivery_fee_cents??0))/100,tax_amount:Number(booking.tax_cents??0)/100,discount_amount:Number(booking.discount_cents??0)/100,discount_snapshot:booking.discount_snapshot,booking_source:"website",estimated_duration_minutes:Math.max(1,Math.round((end.getTime()-start.getTime())/60000)),payment_status:Number(booking.amount_paid_cents||0)>0?(Number(booking.balance_due_cents||0)>0?"partially_paid":"paid"):"unpaid",request_key:booking.id};
 ...
 if(jobError)throw new RentalBookingJobPersistenceError("create_rental_job","jobs",jobError);
 if(!job)throw new Error("Rental job could not be created (no row returned).");
 const linked=await db.from("bookings").update({job_id:job.id}).eq("id",booking.id).is("job_id",null);
 if(linked.error)throw new RentalBookingJobPersistenceError("link_rental_job","bookings",linked.error);
 return job.id;
}
