...
export async function ensureRentalBookingJob(db:SupabaseClient,bookingId:string){
 ...
 const itemSummary=items.map(row=>{const item=Array.isArray(row.inventory_items)?row.inventory_items[0]:row.inventory_items;return `${item?.name??"Rental item"} × ${row.quantity}`;});
