...
export async function loadBusinessWebsiteData(db:SupabaseClient,settings:WebsiteRow):Promise<BusinessSiteData|null>{
 ...
 const googleRating=settings.google_place_id?await getGoogleBusinessRating(String(settings.google_place_id)):null;
 return {
  ...
  googleReviewUrl:googleRating?.googleMapsUri??settings.google_review_url,googleRating:googleRating?.rating??null,googleReviewCount:googleRating?.reviewCount??null,googleReviews:(Array.isArray(settings.google_reviews)?settings.google_reviews:[]).filter((review:any)=>review&&typeof review.author==="string"&&typeof review.text==="string"&&Number.isInteger(review.rating)&&review.rating>=1&&review.rating<=5).slice(0,6),photoUrls:(settings.photo_urls??[]).filter(Boolean),requestEnabled:settings.request_service_enabled??true,
