...
const readOnlyDeliveryQuestion=/^\s*(?:when\s+(?:did|was)|did\s+(?:i|we|they)|was\s+(?:it|that|the)|has\s+(?:it|that|the)|how\s+many\s+times|what\s+(?:date|happened)|who|where|why)\b/i;
