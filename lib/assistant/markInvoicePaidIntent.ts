...
const action=/^\s*(?:(?:please|kindly)\s+)?(?:(?:mark|set|record)\b.*\b(?:paid|as paid)\b|(?:they|he|she|customer)\s+paid\b.*\binvoice\b|(?:this|that|the)\s+invoice\s+has\s+been\s+paid\b)/i;
