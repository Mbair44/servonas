...
export function selectCustomerConversationContext<T extends Record<string,unknown>>(context:T,customerId:string){const next={...context,selectedCustomerId:customerId};if(context.selectedCustomerId!==customerId)delete (next as Record<string,unknown>).selectedInvoiceId;delete (next as Record<string,unknown>).pendingSelection;delete (next as Record<string,unknown>).pendingCustomerCandidates;return next as T&{selectedCustomerId:string};}
...
export function clearSelectedCustomerConversationContext<T extends Record<string,unknown>>(context:T){const next={...context};delete next.selectedCustomerId;delete next.selectedInvoiceId;delete next.pendingSelection;delete next.pendingCustomerCandidates;return next;}
