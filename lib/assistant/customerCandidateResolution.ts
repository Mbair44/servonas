...
export function selectCustomerConversationContext<T extends Record<string,unknown>>(context:T,customerId:string){const changed=context.selectedCustomerId!==customerId,next=changed?clearCustomerChildConversationContext({...context,selectedCustomerId:customerId}):{...context,selectedCustomerId:customerId};if(changed)delete (next as Record<string,unknown>).selectedJobId;delete (next as Record<string,unknown>).pendingSelection;delete (next as Record<string,unknown>).pendingCustomerCandidates;return next as T&{selectedCustomerId:string};}
