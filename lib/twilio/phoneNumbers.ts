...
export type PhoneNumberProvider={search(accountSid:string,areaCode:string,capabilities:NumberCapabilities):Promise<AvailablePhoneNumber[]>;findOwned(accountSid:string,phone:string):Promise<ProviderNumber|null>;purchase(accountSid:string,phone:string,friendlyName:string):Promise<ProviderNumber>};

export function getTwilioPhoneNumberProvider(http:TwilioHttpClient=getParentTwilioHttpClient()):PhoneNumberProvider{
 ...
 return {
  async search(sid,area,caps){const q=new URLSearchParams({AreaCode:area,SmsEnabled:"true",PageSize:"20"});if(caps.mms)q.set("MmsEnabled","true");if(caps.voice)q.set("VoiceEnabled","true");const v=await http.request<{available_phone_numbers?:ProviderNumber[]}>(`${base(sid)}/AvailablePhoneNumbers/US/Local.json?${q}`);return (v.available_phone_numbers??[]).map(convert);},
  async findOwned(sid,phone){const q=new URLSearchParams({PhoneNumber:phone,PageSize:"20"});const v=await http.request<{incoming_phone_numbers?:ProviderNumber[]}>(`${base(sid)}/IncomingPhoneNumbers.json?${q}`);return v.incoming_phone_numbers?.find(n=>n.phone_number===phone)??null;},
  purchase:(sid,phone,name)=>http.request(`${base(sid)}/IncomingPhoneNumbers.json`,formRequest({PhoneNumber:phone,FriendlyName:name})),
 };
}
...
export function createPhoneNumberService(deps:{repository:PhoneNumberRepository;provider:PhoneNumberProvider;getAccount:typeof getBusinessTwilioContext;getCompliance:typeof getBusinessComplianceStatus}){
 const requireEligible=async(businessId:string)=>{const account=await deps.getAccount(businessId);if(!account?.subaccountSid||account.provisioningStatus!=="active")throw new Error("The business needs an active Twilio subaccount first.");const compliance=await deps.getCompliance(businessId);if(compliance?.status!=="approved")throw new Error("Twilio compliance must be approved before selecting a number.");return account;};
 return {
  async searchBusinessPhoneNumbers(businessId:string,options:{areaCode:string;fallbackAreaCodes?:string[];mms?:boolean;voice?:boolean}){const account=await requireEligible(businessId);const areas=[options.areaCode,...(options.fallbackAreaCodes??[])].filter((v,i,a)=>/^\d{3}$/.test(v)&&a.indexOf(v)===i);if(!areas.length)throw new Error("A valid US area code is required.");const caps={sms:true,mms:Boolean(options.mms),voice:Boolean(options.voice)};for(const area of areas){const found=await deps.provider.search(account.subaccountSid!,area,caps);if(found.length)return found;}return[];},
  async purchaseBusinessPhoneNumber(businessId:string,phoneNumber:string,metadata?:Partial<AvailablePhoneNumber>){
   if(!e164(phoneNumber))throw new Error("A valid E.164 phone number is required.");const account=await requireEligible(businessId);const primary=await deps.repository.getPrimary(businessId);if(primary?.twilio_phone_number_sid)return primary;
   ...
   try{const owned=await deps.provider.findOwned(account.subaccountSid!,phoneNumber);const purchased=owned??await deps.provider.purchase(account.subaccountSid!,phoneNumber,metadata?.friendlyName??`Servonas ${businessId}`);if(!purchased.sid)throw new Error("Twilio returned no phone-number SID.");
    // Do not set SmsUrl yet: Twilio signs subaccount webhooks with that subaccount's
    // Auth Token, and Phase 1 deliberately discarded it. Enabling the URL before a
    // secure secret resolver exists would either reject valid traffic or weaken auth.
    return deps.repository.mark(row.id,{twilio_phone_number_sid:purchased.sid,provisioning_status:"pending_webhook_security",provisioning_error:"Inbound SMS activation awaits secure subaccount webhook-token retrieval.",last_synced_at:new Date().toISOString()});
...
const service=()=>createPhoneNumberService({repository:repository(),provider:getTwilioPhoneNumberProvider(),getAccount:getBusinessTwilioContext,getCompliance:getBusinessComplianceStatus});
