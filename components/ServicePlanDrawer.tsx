 ...
 <ManagementDrawer open={open} title={generatedName} onDirty={()=>{}} onClose={()=>setOpen(false)} size="wide">
  <form action={action} className="quick-employee-form service-plan-form service-plan-create-form">
   ...
   <fieldset className="service-plan-overview">
    ...
    <label className="service-plan-effective-date"><span className="service-plan-label-title">Start date <b>*</b></span><input name="startDate" type="date" required value={effective} onChange={event=>setEffective(event.target.value)}/></label>
    <label className="service-plan-end-date"><span className="service-plan-label-title">End date <small>Optional</small></span><input name="endDate" type="date"/></label>
   </fieldset>
