...
export function IndustryLandingPage({config}:{config:IndustryLandingConfig}){
 return <main className="marketing-home industry-landing">
  <section className="home-hero"><div className="home-hero-glow one"/><div className="home-hero-glow two"/><div className="sv-container industry-hero-grid"><div className="home-hero-copy"><span className="sv-kicker">{config.kicker}</span><h1>{config.headline} <span>{config.accent}</span></h1><p>{config.description}</p><div className="sv-actions"><Link className="sv-button home-primary-cta" href="/signup">Start Free <span aria-hidden="true">→</span></Link>{config.showDemoCta!==false&&<Link className="sv-button sv-secondary" href="/demo">See Servonas in action</Link>}</div><div className="home-proof">{config.proof.map(item=><span key={item}>✓ {item}</span>)}</div></div><aside className={`industry-hero-card${config.heroImage?" has-image":""}`}>{config.heroImage&&<div className="industry-hero-photo"><Image src={config.heroImage.src} alt={config.heroImage.alt} fill priority sizes="(max-width: 1000px) 100vw, 42vw"/></div>}<span>Built for {config.industry}</span><h2>Your day, organized.</h2>{config.workflow.slice(0,4).map((item,index)=><div key={item}><i>{index+1}</i><strong>{item}</strong><b>Ready</b></div>)}</aside></div></section>
