...
export function IndustryLandingPage({config}:{config:IndustryLandingConfig}){
 return <main className="marketing-home industry-landing">
  <section className="home-hero"><div className="home-hero-glow one"/><div className="home-hero-glow two"/><div className="sv-container industry-hero-grid"><div className="home-hero-copy"><span className="sv-kicker">{config.kicker}</span><h1>{config.headline} <span>{config.accent}</span></h1><p>{config.description}</p><div className="sv-actions"><Link className="sv-button home-primary-cta" href="/signup">Start Free <span aria-hidden="true">→</span></Link><Link className="sv-button sv-secondary" href="/demo">See Servonas in action</Link></div><div className="home-proof">{config.proof.map(item=><span key={item}>✓ {item}</span>)}</div></div><aside className="industry-hero-card"><span>Built for {config.industry}</span><h2>Your day, organized.</h2>{config.workflow.slice(0,4).map((item,index)=><div key={item}><i>{index+1}</i><strong>{item}</strong><b>Ready</b></div>)}</aside></div></section>
