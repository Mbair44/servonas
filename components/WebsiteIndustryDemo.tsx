...
export function WebsiteIndustryDemo({config,params}:{config:DemoConfig;params:Record<string,string|string[]|undefined>}){
 ...
 const site:BusinessSiteData={name:config.businessName,phone:"(555) 010-0148",email:"hello@example.invalid",logoUrl:null,industryProfile:config.industryProfile,websiteSource:config.source,template:"modern",primaryColor:config.primary,secondaryColor:config.secondary,heroHeading:config.hero,heroSubheading:config.subheading,aboutText:config.about,instagramUrl:null,googleReviewUrl:null,googleRating:null,googleReviewCount:null,googleReviews:[],photoUrls:config.photos??[],requestEnabled:false,bookingEnabled:false,bookingUrl:null,services:config.services.map((name,index)=>({id:String(index+1),name,description:`Professional ${name.toLowerCase()} from a dependable local team.`,price_amount:null,price_label:"quote"})),rentalItems:[],hours:[1,2,3,4,5].map(weekday=>({weekday,start:"08:00",end:"17:00"})),serviceAreas:config.areas,announcementText:null};
 return <><aside className="industry-example-banner"><span>This is a fictional example website built with Servonas.</span><Link href={`/signup?${query}`}>Build My Website — Free</Link></aside><BusinessWebsite site={site}/></>;
}
