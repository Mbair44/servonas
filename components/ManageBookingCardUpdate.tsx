...
function CardForm({ token, clientSecret, onCancel, onSaved, onLoadError, onReady }: { token: string; clientSecret: string; onCancel: () => void; onSaved: (brand?: string | null, last4?: string | null) => void; onLoadError: () => void; onReady: () => void }) {
  ...
  return <form className="manage-booking-card-form" onSubmit={async event => {
    ...
    onSaved(data.brand, data.last4);
  }}>
    <strong className="manage-booking-card-form-title">Use a different card</strong><p>Enter the card you&apos;d like us to use for the scheduled balance payment.</p>
...
export function ManageBookingCardUpdate({ token }: { token: string }) {
  const [setup, setSetup] = useState<Setup | null>(null); const [opening, setOpening] = useState(false); const [error, setError] = useState(""); const [success, setSuccess] = useState<{ brand?: string | null; last4?: string | null } | null>(null); const [elementState, setElementState] = useState<ElementState>("loading");
  ...
  return <div className="manage-booking-card-update">
    ...
    {setup && stripePromise && <Elements stripe={stripePromise} options={{ clientSecret: setup.clientSecret }}><CardForm token={token} clientSecret={setup.clientSecret} onCancel={close} onSaved={(brand, last4) => { close(); setSuccess({ brand, last4 }); }} onReady={() => setElementState("ready")} onLoadError={() => setElementState("failed")}/></Elements>}
    {elementState === "loading" && setup && <p role="status">Loading secure card form...</p>}{elementState === "failed" && <p role="alert">We couldn&apos;t load the secure card form. Please refresh and try again.</p>}{error && <p role="alert">{error}</p>}{success && <div className="manage-booking-card-success" role="status"><strong>✓ Payment method updated</strong><span>We&apos;ll use this card for your scheduled balance payment.</span>{success.brand && success.last4 && <small>{String(success.brand).replace(/^./, char => char.toUpperCase())} •••• {success.last4}</small>}</div>}
  </div>;
}
