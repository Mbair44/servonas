...
function CardForm({ token, onCancel, onSaved, onLoadError, onReady }: { token: string; onCancel: () => void; onSaved: (message: string) => void; onLoadError: () => void; onReady: () => void }) {
  const stripe = useStripe(); const elements = useElements(); const [error, setError] = useState(""); const [saving, setSaving] = useState(false); const [elementReady, setElementReady] = useState(false);
  return <form className="manage-booking-card-form" onSubmit={async event => {
    ...
    <div className="manage-booking-card-element"><PaymentElement onReady={() => { setElementReady(true); onReady(); }} onLoadError={onLoadError}/></div>
...
export function ManageBookingCardUpdate({ token }: { token: string }) {
  ...
  useEffect(() => {
    if (!setup || elementState !== "loading") return;
    const timeout = window.setTimeout(() => setElementState(current => current === "loading" ? "failed" : current), 10000);
    return () => window.clearTimeout(timeout);
  }, [setup, elementState]);
  const open = async () => {
    if (opening) return; setOpening(true); setError(""); setSuccess("");
    try { const response = await fetch(`/api/manage-booking/${token}/setup-intent`, { method: "POST" }); const data = await response.json(); if (!data.clientSecret || !data.publishableKey || !data.connectedAccount) { setError(data.error ?? "We couldn't open a secure card form right now."); return; } setElementState("loading"); setSetup(data); }
    catch { setError("We couldn't open a secure card form right now."); }
  ...
  return <div className="manage-booking-card-update">
    ...
    {setup && stripePromise && <Elements stripe={stripePromise} options={{ clientSecret: setup.clientSecret }}><CardForm token={token} onCancel={close} onSaved={message => { close(); setSuccess(message); }} onReady={() => setElementState("ready")} onLoadError={() => setElementState("failed")}/></Elements>}
    {elementState === "loading" && setup && <p role="status">Loading secure card form...</p>}
    {elementState === "failed" && <p role="alert">We couldn&apos;t load the secure card form. Please refresh and try again.</p>}
