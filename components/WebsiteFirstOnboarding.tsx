...
export function WebsiteFirstBusiness({defaultEmail="",source}:{defaultEmail?:string;source:WebsiteFirstSource}){
 ...
 return <div className="website-first-shell">
  ...
  <section><span className="sv-kicker">Your {config.industryLabel} website</span><h1>{config.businessHeading}</h1><p>Give us the basics and we&apos;ll build your first website preview. You can fine-tune everything later.</p>{state.error&&<p className="auth-error">{state.error}</p>}
   <form action={action} className="website-first-form website-first-business-form"><input type="hidden" name="source" value={source}/><input type="hidden" name="slug" value={slug||clean(name)||"new-business"}/><p className="wide website-required-note"><span>*</span> Required fields</p>
