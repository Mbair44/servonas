...
export function WebsiteFirstBusiness({defaultEmail="",source}:{defaultEmail?:string;source:WebsiteFirstSource}){
 ...
 const draftSlug=slug||clean(name)||"new-business";
 ...
 return <div className="website-first-shell">
  ...
  <section><AcquisitionFunnelTracker industry={source} event="website_builder_step1_started"/><span className="sv-kicker">Your {config.industryLabel} website</span><h1>{config.businessHeading}</h1><p>Give us the basics and we&apos;ll build your website. You can publish it first and improve anything later.</p><div className="website-first-reassurance"><span>Takes about 60 seconds</span><span>No credit card required</span></div>{state.error&&<p className="auth-error">{state.error}</p>}
  <form action={async(formData)=>{formData.set("acquisitionSessionId",acquisitionSessionId(source));trackAcquisition(source,"website_preview_generation_started");await action(formData);}} className="website-first-form website-first-business-form"><input type="hidden" name="source" value={source}/><input type="hidden" name="slug" value={draftSlug}/><input type="hidden" name="domainPreference" value="existing_domain"/><input type="hidden" name="domainName" value={`${draftSlug}.servonas-temp.com`}/><p className="wide website-required-note"><span>*</span> Required fields</p>
    ...
    <button className="sv-button wide website-first-preview-cta" disabled={pending}>{pending?"Saving your business details…":"Continue to Style →"}</button><p className="wide website-first-next">Next: Pick a look, then we&apos;ll build your website.</p>
   </form>
  </section>
 </div>;
}
