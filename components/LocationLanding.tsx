...
export function LocationLanding({page,business,websiteUrl,ctaUrl,nearbyPages=[],categoryPages=[]}:{page:Page;business:Business;websiteUrl:string;ctaUrl:string;nearbyPages:Array<{slug:string;city:string;state:string|null}>;categoryPages?:Array<{slug:string;title:string}>}){
 ...
 return <main className={`promotion-landing location-landing ${styles.page}`} style={{"--promotion-brand":business.primaryColor??business.primary_color??"#1769f5"} as React.CSSProperties}>
  ...
  {categoryPages.length>0&&<nav className="location-page-categories" aria-label="Rental categories"><strong>Explore rental categories</strong>{categoryPages.map(category=><Link href={category.slug} key={category.slug}>{category.title}</Link>)}</nav>}
