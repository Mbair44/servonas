...
export function WebsiteRequestForm({action,services,businessName,businessSlug,variant="service"}:{action:(state:WebsiteRequestState,data:FormData)=>Promise<WebsiteRequestState>;services:Service[];businessName:string;businessSlug?:string;variant?:"service"|"quote"}){
 ...
 return <form action={async(formData)=>{if(businessSlug)trackBookingFunnel(businessSlug,"lead_submitted",{metadata:{mode:quoteMode?"quote_request":"service_request",surface:"public_request_form"}});await formAction(formData);}} className="business-site-request-form" onFocus={()=>{if(businessSlug)trackBookingFunnel(businessSlug,"booking_cta_click",{metadata:{mode:quoteMode?"quote_request":"service_request",surface:"public_request_form"}});}}>
