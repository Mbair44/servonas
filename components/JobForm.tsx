...
export default function JobForm({
  ...
  return <form id={id} action={formAction} className={`job-form${onCancel?" job-drawer-form":""}`}>
    ...
    <fieldset className="job-form-section job-details-section"><legend><i><JobSectionIcon name="details"/></i><span><strong>Job details</strong><small>Add the basics about this job.</small></span></legend><div className="job-form-grid">
      <label className="wide">Job title <b className="required-mark">Required</b><input required name="title" defaultValue={value("title", String(job?.title ?? ""))} placeholder="AC repair, landscape cleanup, annual inspection…"/>{error("title")}</label>
      <label>Customer <b className="required-mark">Required</b><select required name="customerId" value={customerId} onChange={(event) => setCustomerId(event.target.value)}><option value="">Choose customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.company_name || `${customer.first_name} ${customer.last_name}`}</option>)}</select>{error("customerId")}</label>
    ...
    <fieldset className="job-form-section job-schedule-section"><legend><i><JobSectionIcon name="schedule"/></i><span><strong>Schedule &amp; dispatch</strong><small>Set appointment time, arrival flexibility, and priority.</small></span></legend><div className="job-form-grid">
      ...
      {isCreate?<><input type="hidden" name="endsAt" value={endsAt}/><input type="hidden" name="arrivalWindowStart" value={arrivalStart}/><input type="hidden" name="arrivalWindowEnd" value={arrivalEnd}/></>:<label className="wide job-time-commitment"><input type="checkbox" checked={fixedTime} onChange={event=>setFixedTime(event.target.checked)}/><span><strong>{fixedTime?"Appointment time is set":"Appointment time is flexible"}</strong><small>{fixedTime?"The technician must arrive at this time. Route calculation cannot move this stop.":"Route calculation may place this job anywhere on the selected service day."}</small></span></label>}
      <label>Scheduled start<input name="startsAt" type="datetime-local" value={startsAt} onChange={event=>setStartsAt(event.target.value)}/>{error("startsAt")}</label>
      {isCreate?<label>Arrival window<select value={arrivalPreset} onChange={event=>setArrivalPreset(event.target.value)}><option value="exact">Exact appointment time</option><option value="30">± 30 minutes</option><option value="60">± 1 hour</option><option value="120">± 2 hours</option><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="custom">Custom</option></select><small>{arrivalPreset==="exact"?"Route calculation cannot move this stop.":"Routing can place the visit within this window."}</small></label>:<><label>Scheduled end<input name="endsAt" type="datetime-local" value={endsAt} onChange={event=>setEndsAt(event.target.value)}/></label><label>Arrival window start<input disabled={!fixedTime} name="arrivalWindowStart" type="datetime-local" defaultValue={value("arrivalWindowStart", String(job?.arrival_window_start_local ?? ""))}/></label><label>Arrival window end<input disabled={!fixedTime} name="arrivalWindowEnd" type="datetime-local" defaultValue={value("arrivalWindowEnd", String(job?.arrival_window_end_local ?? ""))}/></label></>}
      {isCreate&&arrivalPreset==="custom"&&<div className="job-custom-window wide"><label>Window start<input type="datetime-local" value={customArrivalStart} onChange={event=>setCustomArrivalStart(event.target.value)}/></label><label>Window end<input type="datetime-local" value={customArrivalEnd} onChange={event=>setCustomArrivalEnd(event.target.value)}/></label></div>}
      <label>Estimated duration (minutes)<input name="estimatedDurationMinutes" type="number" min="1" max="10080" value={duration} onChange={event=>setDuration(event.target.value)} placeholder="60"/></label>
      ...
      {!isCreate&&<><label>Status<select name="status" defaultValue={value("status", String(job?.status ?? "draft"))}>{jobStatuses.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select>{error("status")}</label><label>Source<input name="source" defaultValue={value("source", String(job?.booking_source ?? "dashboard"))}/></label></>}
    ...
    <fieldset className="job-form-section job-notes-section"><legend><i><JobSectionIcon name="notes"/></i><span><strong>Notes</strong><small>Record the scope and choose what the customer can see.</small></span></legend><div className="job-form-grid job-notes-grid">
      <label>Description<textarea name="description" rows={3} defaultValue={value("description", String(job?.description ?? ""))} placeholder="What work will be done?"/></label>
