...
export default function JobForm({
  ...
  return <form id={id} action={formAction} className={`job-form${onCancel?" job-drawer-form":""}`}>
    ...
    <fieldset className="job-form-section job-schedule-section"><legend><i><JobSectionIcon name="schedule"/></i><span><strong>Schedule &amp; dispatch</strong><small>Set appointment time, arrival flexibility, and priority.</small></span></legend><div className="job-form-grid">
      ...
      <label>Scheduled start<input name="startsAt" type="datetime-local" value={startsAt} onChange={event=>{setStartsAt(event.target.value);updateValue("startsAt",event.target.value);}}/>{error("startsAt")}</label>
      {isCreate&&<label>Scheduled end<input name="endsAt" type="datetime-local" value={endsAt} onChange={event=>{setEndsAt(event.target.value);updateValue("endsAt",event.target.value);}}/><small>Required when you set a scheduled start.</small></label>}
      {isCreate?<label>Arrival window<select value={arrivalPreset} onChange={event=>setArrivalPreset(event.target.value)}><option value="exact">Exact appointment time</option><option value="30">± 30 minutes</option><option value="60">± 1 hour</option><option value="120">± 2 hours</option><option value="morning">Morning</option><option value="afternoon">Afternoon</option><option value="custom">Custom</option></select><small>{arrivalPreset==="exact"?"Route calculation cannot move this stop.":"Routing can place the visit within this window."}</small></label>:<><label>Scheduled end<input name="endsAt" type="datetime-local" value={endsAt} onChange={event=>{setEndsAt(event.target.value);updateValue("endsAt",event.target.value);}}/></label><label>Arrival window start<input disabled={!fixedTime} name="arrivalWindowStart" type="datetime-local" value={value("arrivalWindowStart", String(job?.arrival_window_start_local ?? ""))} onChange={event=>updateValue("arrivalWindowStart",event.target.value)}/></label><label>Arrival window end<input disabled={!fixedTime} name="arrivalWindowEnd" type="datetime-local" value={value("arrivalWindowEnd", String(job?.arrival_window_end_local ?? ""))} onChange={event=>updateValue("arrivalWindowEnd",event.target.value)}/></label></>}
