...
export function EmbeddedBookingFrame({src,title}:{src:string;title:string}){
 ...
 useEffect(()=>{
  const message=(event:MessageEvent)=>{if(event.source!==frame.current?.contentWindow||event.data?.type!=="servonas:booking-height")return;const next=Number(event.data.height);if(Number.isFinite(next)&&next>=400&&next<=10000)setHeight(Math.ceil(next));};
