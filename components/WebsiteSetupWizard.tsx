...
const steps=[
 {key:"basic",title:"Basic Info",subtitle:"Your core message and business details"},
 {key:"design",title:"Design",subtitle:"Template, logo, colors, and photos"},
 {key:"services",title:"Services",subtitle:"What customers see on your website"},
 {key:"hours",title:"Hours & Areas",subtitle:"Useful details for local visitors"},
 {key:"features",title:"Features",subtitle:"Requests, booking, reviews, and social"},
 {key:"domain",title:"Domain",subtitle:"Your custom website address"},
 {key:"review",title:"Review & Publish",subtitle:"Go live or keep improving"},
] as const;
...
export function WebsiteSetupWizard({children,previewUrl,initialStep="basic",checks,publishControl}:{children:ReactNode;previewUrl:string;initialStep?:string;checks:Check[];publishControl?:ReactNode}){
 ...
 return <div className={`website-builder-shell website-step-${active}`}>
  <aside className="website-setup-nav" aria-label="Website setup steps"><div><strong>Improve your website</strong><span>Your website can be published first and refined anytime.</span></div><ol>{steps.map((step,stepIndex)=>{const complete=stepIndex<index||checks[stepIndex]?.complete;return <li key={step.key}><button type="button" className={active===step.key?"active":complete?"complete":""} onClick={()=>choose(step.key)} aria-current={active===step.key?"step":undefined}><i>{complete?<WebsiteIcon name="check"/>:stepIndex+1}</i><span><strong>{step.title}</strong><small>{step.subtitle}</small></span></button></li>;})}</ol></aside>
  <section className="website-step-editor"><header><span>Enhancement {index+1} of {steps.length}</span><h2>{steps[index].title}</h2><p>{steps[index].subtitle}</p><button type="button" className="sv-button sv-secondary website-mobile-preview" onClick={openPreview}><WebsiteIcon name="eye"/> Preview</button></header>{active==="review"&&<><div className="website-review-summary"><h3>Publish now or keep improving</h3><p>Nothing here has to block launch. Review the essentials below and jump into any area you want to improve.</p>{checks.map((check,checkIndex)=><button type="button" key={check.label} onClick={()=>choose(steps[Math.min(checkIndex,steps.length-1)].key)}><i className={check.complete?"complete":"attention"}><WebsiteIcon name={check.complete?"check":"alert"}/></i><span><strong>{check.label}</strong><small>{check.detail}</small></span><b>{check.complete?"Ready":"Optional"}</b></button>)}</div>{publishControl&&<div className="website-review-publish">{publishControl}</div>}</>}{children}</section>
