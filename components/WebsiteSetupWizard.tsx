...
export function WebsiteSetupWizard({children,previewUrl,initialStep="basic",checks}:{children:ReactNode;previewUrl:string;initialStep?:string;checks:Check[]}){
 const valid=steps.some(step=>step.key===initialStep),[active,setActive]=useState<StepKey>((valid?initialStep:"basic") as StepKey),[device,setDevice]=useState<"desktop"|"tablet"|"mobile">("desktop"),[previewOpen,setPreviewOpen]=useState(false);
 ...
 return <div className={`website-builder-shell website-step-${active}`}>
  ...
  <section className="website-step-editor"><header><span>Step {index+1} of {steps.length}</span><h2>{steps[index].title}</h2><p>{steps[index].subtitle}</p><button type="button" className="sv-button sv-secondary website-mobile-preview" onClick={()=>setPreviewOpen(true)}><WebsiteIcon name="eye"/> Preview</button></header>{active==="review"&&<div className="website-review-summary"><h3>Ready to publish?</h3><p>Review the essentials below. You can return to any step to make changes.</p>{checks.map((check,checkIndex)=><button type="button" key={check.label} onClick={()=>choose(steps[Math.min(checkIndex,steps.length-1)].key)}><i className={check.complete?"complete":"attention"}><WebsiteIcon name={check.complete?"check":"alert"}/></i><span><strong>{check.label}</strong><small>{check.detail}</small></span><b>{check.complete?"Complete":"Needs attention"}</b></button>)}</div>}{children}</section>
  <aside className={`website-live-preview ${previewOpen?"open":""}`}><header><div><strong>Live preview</strong><span>Saved website settings</span></div><nav aria-label="Preview size">{(["desktop","tablet","mobile"] as const).map(item=><button type="button" key={item} className={device===item?"active":""} onClick={()=>setDevice(item)} aria-label={`${item} preview`}>{item.slice(0,1).toUpperCase()}</button>)}</nav><button type="button" className="website-preview-close" onClick={()=>setPreviewOpen(false)} aria-label="Close preview">×</button></header><div className={`website-preview-frame ${device}`}><iframe src={previewUrl} title="Website live preview"/></div></aside>
 </div>;
}
