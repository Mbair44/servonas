...
export function WebsiteHoursAreasFields({hours,areas,disabled}:{hours:HoursRow[];areas:AreaRow[];disabled:boolean}){
 ...
 return <section className="workspace-panel website-setting-section website-hours-areas-section">
  ...
  <div className="website-inline-hours"><BusinessHoursEditor initialHours={hours.length?Array.from({length:7},(_,weekday)=>{const row=byDay.get(weekday);return {weekday,open:Boolean(row),start:row?.start_time?.slice(0,5)??"09:00",end:row?.end_time?.slice(0,5)??"17:00"};}):undefined} disabled={disabled}/></div>
