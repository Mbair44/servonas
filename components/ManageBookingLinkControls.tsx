...
export function ManageBookingLinkControls({ slug, jobId, status, recentSessions=[] }: { slug: string; jobId: string; status: string; recentSessions?:SessionOption[] }) {
  ...
  const paymentRequest = () => start(async () => {
    try {
      ...
      setNotice(`Booking held until ${new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "short" }).format(new Date(result.expiresAt))}. Payment request ${result.smsSent ? `sent to ${masked}` : "created"}.`);
