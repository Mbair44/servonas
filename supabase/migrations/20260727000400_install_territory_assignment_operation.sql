...
do $$
begin
  if to_regclass('public.employee_territory_assignments') is null
    or to_regclass('public.workforce_territories') is null
    or to_regclass('public.employees') is null then
    raise exception 'Epic 8.5 Checkpoint 5 requires the Epic 8 Checkpoint 4 territory tables';
  end if;
  if exists(
    ...
    raise exception 'Multiple active primary employees exist for a territory. Resolve the Checkpoint 5 audit before installing assignment operations.'
      using errcode='23505';
  end if;
end
$$;
...
alter table public.employee_territory_assignments
  drop constraint if exists employee_territories_type_check;
alter table public.employee_territory_assignments
  ...
  from (
    select territory.name,
      min(case assignment.assignment_type
        when 'primary' then 1 when 'backup' then 2
        when 'secondary' then 3 else 4
    ...
    join public.workforce_territories territory
      ...
      and territory.id=assignment.territory_id and territory.is_active
