...
begin
  ...
  select min(i.business_id::text)::uuid into v_business_id
  from requested_items r join public.inventory_items i on i.id=r.inventory_item_id;
  if v_business_id is null or (select count(distinct i.business_id) from requested_items r join public.inventory_items i on i.id=r.inventory_item_id) <> 1 then
    raise exception 'All rental items must belong to the same business.';
  end if;
  ...
  insert into public.bookings (
    business_id, customer_id, status, event_start_time, event_end_time,
    delivery_address, delivery_city, delivery_state, delivery_zip,
    notes, subtotal_cents, tax_cents, total_cents, agreement_accepted_at
  ) values (
    v_business_id, v_customer_id, 'pending_payment', p_event_start_time, p_event_end_time,
