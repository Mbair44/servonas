-- The booking total is the authoritative final sale. Some historical bookings
-- already store a discounted booking-item price while retaining the original
-- order discount, so subtracting discount_cents from the line again undercounts.
...
language sql stable security invoker set search_path=public as $$
 with completed_lines as (
  select bi.id booking_item_id,bi.inventory_item_id listing_inventory_item_id,b.id booking_id,
   ...
   greatest(coalesce(b.total_cents,0)-coalesce(b.delivery_fee_cents,0)-coalesce(b.tax_cents,0)-coalesce(b.operator_total_cents,0),0)::numeric final_rental_revenue_cents
  ...
  select booking_id,sum(item_gross_cents)::numeric rental_gross_cents,max(final_rental_revenue_cents)::numeric final_rental_revenue_cents
  ...
  select line.booking_item_id,line.listing_inventory_item_id,line.booking_id,round(case when totals.rental_gross_cents<=0 then 0 else
   line.item_gross_cents/totals.rental_gross_cents*totals.final_rental_revenue_cents end)::bigint final_sale_cents
