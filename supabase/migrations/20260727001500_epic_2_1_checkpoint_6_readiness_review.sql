...
begin
 ...
 if not exists(select 1 from public.business_entitlements where business_id=p_business_id and entitlement_key='pilot' and status='active' and starts_at<=now() and (ends_at is null or ends_at>now())) then raise exception 'Pilot access is inactive' using errcode='42501';end if;
