...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'customerKey',case when nullif(lower(btrim(c.email)),'') is not null
   then 'email:'||md5(nullif(lower(btrim(c.email)),''))
...
revoke all on function public.financial_dashboard_summary(uuid,date),public.financial_collected_payment_details(uuid),public.financial_collected_payments(uuid),public.sales_performance_summary(uuid,date,date),public.sales_performance_details(uuid,date,date) from public;
grant execute on function public.financial_dashboard_summary(uuid,date),public.financial_collected_payment_details(uuid),public.financial_collected_payments(uuid),public.sales_performance_summary(uuid,date,date),public.sales_performance_details(uuid,date,date) to authenticated,service_role;
