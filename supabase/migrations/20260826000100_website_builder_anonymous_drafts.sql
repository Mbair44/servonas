...
create table if not exists public.website_builder_drafts (
  ...
  acquisition_session_id uuid null references public.website_acquisition_sessions(id) on delete set null,
  email text null,
  ...
  constraint website_builder_drafts_source_check check (
    source in (
      ...
      'floral-event-website',
      'event-party-rentals-website'
    )
  ),
  constraint website_builder_drafts_step_check check (
    current_step in ('business','style','preview','claimed','completed')
  )
);

create index if not exists website_builder_drafts_business_idx
  on public.website_builder_drafts(business_id);

create index if not exists website_builder_drafts_claimed_idx
  on public.website_builder_drafts(claimed_by_user_id, expires_at desc);

alter table public.website_builder_drafts enable row level security;

drop policy if exists "service role manages website builder drafts" on public.website_builder_drafts;
create policy "service role manages website builder drafts"
on public.website_builder_drafts
for all
to service_role
using (true)
with check (true);
...
declare
 ...
 v_entitlement public.business_entitlements;
 v_primary text;
 v_secondary text;
 v_hero text;
 v_subheading text;
 v_about_suffix text;
 v_industry text;
 v_industry_other text;
 v_defaults jsonb:='{}'::jsonb;
begin
 if p_source not in('pest-control-website','car-detailing-website','hvac-website','plumbing-website','landscaping-website','cleaning-website','powerwashing-website','floral-event-website','event-party-rentals-website') then raise exception 'Invalid acquisition source' using errcode='22023';end if;
 ...
 values(btrim(p_name),btrim(p_name),lower(btrim(p_slug)),null,'services',lower(btrim(p_email)),btrim(p_phone),btrim(p_city),upper(btrim(p_state)),'US','America/Phoenix',v_primary,'["booking","customers","team"]',v_industry,v_industry_other,'appointment_service',v_defaults,null)
 ...
 values(v_business.id,'in_progress',4,array['welcome','company','profile'],now(),now(),null);

 insert into public.business_entitlements(business_id,entitlement_key,status,source,metadata,created_by,updated_by)
 values(v_business.id,'pilot','active','pilot',jsonb_build_object('provisioned_by','website_first_onboarding','draft',true),null,null)
 returning * into v_entitlement;

 insert into public.business_onboarding_audit_events(business_id,event_type,actor_user_id,step_key,status,metadata)
 values(v_business.id,'step_completed',null,'profile','in_progress',jsonb_build_object('source',p_source,'completed_steps',array['welcome','company','profile'],'draft',true));
 ...
 values(v_business.id,v_entitlement.id,'provisioned',null,'pilot','active',v_entitlement.metadata);

 insert into public.business_website_onboarding_states(business_id,source,current_step,selected_services,domain_preference,domain_name,updated_by)
 values(v_business.id,p_source,'style',coalesce(p_services,'{}'),p_domain_preference,case when p_domain_preference='existing_domain' then lower(p_domain_name) else null end,null);
 ...
 values(v_business.id,v_business.slug,'draft','modern',v_primary,v_secondary,v_hero,coalesce(nullif(btrim(p_description),''),v_subheading),coalesce(nullif(btrim(p_description),''),v_business.name||v_about_suffix),true,false,null);
 ...
 select v_business.id,btrim(service),'Professional '||lower(btrim(service))||' from '||v_business.name||'.',60,null,'quote',true,'{}',true,null,null
 from unnest(coalesce(p_services,'{}')) service
 where length(btrim(service)) between 2 and 150;

 if nullif(btrim(p_service_area),'') is not null then
  insert into public.workforce_territories(business_id,name,territory_type,postal_codes,neighborhoods,is_active,created_by,updated_by)
  values(v_business.id,btrim(p_service_area),'mixed','{}','{}',true,null,null);
 end if;

 return v_business;
end$$;

revoke all on function public.create_anonymous_website_first_workspace(text,text,text,text,text,text,text,text,text[],text,text,text,text) from public;
grant execute on function public.create_anonymous_website_first_workspace(text,text,text,text,text,text,text,text,text[],text,text,text,text) to service_role;
