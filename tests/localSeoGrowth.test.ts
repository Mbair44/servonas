...
test("the next action advances from discovery to a city promotion",()=>{
 ...
 const promotion=buildLocalGrowthPlan({...base,googleBusinessConnected:true,googleAdsConnected:true});
...
test("unsupported service areas block promotion recommendations",()=>{
 const plan=buildLocalGrowthPlan({...base,serviceAreaSupported:false,googleBusinessConnected:true,googleAdsConnected:true});
