...
test("server speech playback is optional, stoppable, and strips table formatting",()=>{assert.equal(speechFriendlyText("**Total** | $20\nhttps://example.com"),"Total , $20 link");});
test("Talk initializes iOS audio output from the original user gesture",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx");assert.match(client,/function startVoiceSession\(\).*tts\.initialize\(\)/);assert.match(client,/ttsInitializedFromUserGesture:true/);});
test("OpenAI TTS uses one retained audio element and transient object URL",async()=>{const provider=await read("lib/assistant/textToSpeech.ts");assert.match(provider,/audio:HTMLAudioElement\|null/);assert.match(provider,/URL\.createObjectURL\(blob\)/);assert.match(provider,/URL\.revokeObjectURL/);assert.match(provider,/fetch\(this\.endpoint/);assert.doesNotMatch(provider,/SpeechSynthesis/);});
test("safe TTS diagnostics cover request and audio playback",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx"),provider=await read("lib/assistant/textToSpeech.ts");for(const event of ["tts_requested","tts_playback_started","tts_playback_failed","tts_fallback_scheduled","tts_fallback_fired"])assert.match(client,new RegExp(event));for(const event of ["tts_request_started","tts_audio_ready","tts_playback_started","tts_playback_ended","tts_playback_error","tts_object_url_revoked"])assert.match(provider,new RegExp(event));assert.doesNotMatch(provider,/emit\([^\n]*\{[^\n]*\btext\s*[:,}]/);});
...
test("successful TTS completion and errors both return voice state to idle",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx"),provider=await read("lib/assistant/textToSpeech.ts");assert.match(client,/onEnd:finish/);assert.match(client,/function finishSpeaking/);assert.match(provider,/audio\.onended=.*settle\(\)/);assert.match(provider,/audio\.onerror=.*handlers\.onError/);assert.match(provider,/catch\(error\)\{[\s\S]*handlers\.onEnd\?\.\(\)/);});

test("a missing Safari audio callback cannot permanently lock the Assistant",()=>{assert.equal(speechPlaybackTimeoutMs("Short answer"),15_000);assert.equal(speechPlaybackTimeoutMs("x".repeat(2000)),180_000);});
