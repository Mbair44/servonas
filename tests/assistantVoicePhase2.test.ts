...
test("Talk initializes Safari speech output from the original user gesture",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx");assert.match(client,/function startVoiceSession\(\).*tts\.initialize\(\)/);assert.match(client,/ttsInitializedFromUserGesture:true/);});
test("Safari voice loading and utterance retention are explicit",async()=>{const provider=await read("lib/assistant/textToSpeech.ts");assert.match(provider,/activeUtterance:SpeechSynthesisUtterance\|null/);assert.match(provider,/window\.speechSynthesis\.onvoiceschanged=/);assert.match(provider,/this\.voices=window\.speechSynthesis\.getVoices\(\)/);assert.match(provider,/this\.activeUtterance=utterance/);assert.match(provider,/if\(this\.activeUtterance===utterance\)this\.activeUtterance=null/);});
test("TTS does not cancel immediately before every new Safari utterance",async()=>{const provider=await read("lib/assistant/textToSpeech.ts");assert.match(provider,/if\(this\.activeUtterance\)window\.speechSynthesis\.cancel\(\)/);assert.doesNotMatch(provider,/this\.stop\(\);\s*const utterance/);});
test("safe TTS diagnostics cover request playback and browser state",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx"),provider=await read("lib/assistant/textToSpeech.ts");for(const event of ["tts_requested","tts_playback_started","tts_playback_failed","tts_fallback_scheduled","tts_fallback_fired"])assert.match(client,new RegExp(event));for(const event of ["voices_loaded","utterance_created","speech_speak_called","utterance_started","utterance_ended","utterance_error"])assert.match(provider,new RegExp(event));assert.match(provider,/speaking:window\.speechSynthesis\.speaking/);assert.match(provider,/pending:window\.speechSynthesis\.pending/);assert.match(provider,/paused:window\.speechSynthesis\.paused/);assert.doesNotMatch(provider,/emit\([^\n]*\{[^\n]*\btext\s*[:,}]/);});

test("successful voice requests deterministically leave thinking when TTS is disabled",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx");assert.match(client,/setVoiceState\("idle"\);[\s\S]*if\(!speakResponses\|\|!ttsSupported\)\{onComplete\?\.\(\);return;\}/);assert.match(client,/finally\{requestInFlight\.current=false;setLoading\(false\);if\(channel==="voice"\)setVoiceState/);});

test("successful TTS completion and errors both return voice state to idle",async()=>{const client=await read("app/app/[businessSlug]/assistant/AssistantClient.tsx"),provider=await read("lib/assistant/textToSpeech.ts");assert.match(client,/onEnd:finish/);assert.match(client,/function finishSpeaking/);assert.match(provider,/utterance\.onend=.*handlers\.onEnd\?\.\(\)/);assert.match(provider,/utterance\.onerror=.*handlers\.onError\?\.\(category\);handlers\.onEnd\?\.\(\)/);assert.match(provider,/catch\(error\).*handlers\.onError\?\.\(category\);handlers\.onEnd\?\.\(\)/);});
