...
test("tenant sender is available but existing campaign delivery remains on legacy routing",async()=>{const sender=await read("../lib/twilio/tenantOutboundSender.ts"),delivery=await read("../lib/communications/customerCampaignDelivery.ts");assert.match(sender,/data\?\.status!=="active"/);assert.match(sender,/outbound_sender_mode!=="messaging_service"/);assert.match(delivery,/getTwilioCredentials/);assert.doesNotMatch(delivery,/resolveTenantOutboundSender|MessagingServiceSid|tenant-message-status/);});
