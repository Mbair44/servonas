...
test("new conversation identity survives popover rerenders and server recovers the latest scoped conversation",async()=>{const popover=await read("../components/AssistantPopover.tsx"),client=await read("../app/app/[businessSlug]/assistant/AssistantClient.tsx"),orchestrator=await read("../lib/assistant/orchestrator.ts");assert.match(popover,/onConversationId=\{setConversationId\}/);assert.match(popover,/key=\{businessSlug\}/);assert.match(client,/onConversationId\?\.\(body\.conversationId\)/);assert.match(orchestrator,/\.eq\("channel",channel\)\.order\("updated_at",\{ascending:false\}\)\.limit\(1\)\.maybeSingle\(\)/);});
test("conversation state save failures cannot display an unpersisted ambiguity prompt",async()=>{const orchestrator=await read("../lib/assistant/orchestrator.ts");assert.match(orchestrator,/saveConversationContext/);assert.match(orchestrator,/Assistant conversation state could not be saved/);});
