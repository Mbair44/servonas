...
test("checkout funnel events are idempotent through the existing event ledger",async()=>{
 ...
 for(const name of ["customer_info_completed","delivery_address_completed","terms_accepted","payment_cta_clicked","payment_started","payment_succeeded","booking_confirmed"])assert.match(events,new RegExp(`"${name}"`));
