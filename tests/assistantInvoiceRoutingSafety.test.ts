...
test("next appointment cannot short circuit to a generic response",()=>assert.equal((bindTrustedSelectedCustomer("When is their next appointment?",{response:"generic"},customerId) as any).toolName,"getCustomer"));
