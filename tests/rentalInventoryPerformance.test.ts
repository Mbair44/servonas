...
test("a $225 rental sold at 50 percent off attributes $112.50 regardless of deposit collection",async()=>{
 const migration=await readFile(new URL("../supabase/migrations/20260921000400_fix_rental_item_final_sale_revenue.sql",import.meta.url),"utf8");
 assert.match(migration,/final_sale_cents/);
 assert.match(migration,/item_gross_cents\/totals\.rental_gross_cents/);
 assert.doesNotMatch(migration,/amount_paid_cents|collected_cents|invoice_collections|booking_total_cents/);
 assert.match(migration,/booking_discount_cents/);
});
