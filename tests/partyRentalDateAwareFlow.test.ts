...
test("party rental booking blocks empty checkout and uses a storefront-style party summary",async()=>{
 ...
 assert.match(source,/function proceedToCheckout\(itemCount:number\)\{setBookingError\(""\);setUpsell\(null\);requestAnimationFrame\(\(\)=>\{setShowCheckout\(true\);trackBookingFunnel\(businessSlug,"booking_started"/);
