...
test("party rental booking blocks empty checkout and uses a storefront-style party summary",async()=>{
 ...
 assert.match(source,/function showReservationPage\(itemCount:number,nextQuantities=quantities\)\{setBookingError\(""\);trackBookingFunnel\(businessSlug,"booking_started"[\s\S]*persistBookingState\(nextQuantities\);[\s\S]*if\(typeof window!=="undefined"&&checkoutUrl&&window\.parent&&window\.parent!==window\)\{const nextUrl=new URL\(resolveAbsoluteUrl\(checkoutUrl\),window\.location\.origin\),sessionId=attributionSessionId\|\|bookingAttributionSession\(businessSlug\),attribution=bookingAttributionValues\(businessSlug\);nextUrl\.searchParams\.set\("cartState",JSON\.stringify\(nextQuantities\)\);nextUrl\.searchParams\.set\("dateState",JSON\.stringify\(\{date,endDate,startTime,endTime\}\)\);nextUrl\.searchParams\.set\("sv_at",sessionId\);for\(const \[key,value\] of Object\.entries\(attribution\)\)if\(value&&!nextUrl\.searchParams\.has\(key\)\)nextUrl\.searchParams\.set\(key,value\);window\.parent\.postMessage\(\{type:"servonas:open-booking-page",url:nextUrl\.toString\(\)\},"\*"\);return;\}setCheckoutNavigationCount\(current=>current\+1\);setShowCheckout\(true\);/);
