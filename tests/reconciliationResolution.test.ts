...
test("Stripe verification is tenant-scoped and read-only",()=>{const source=readFileSync(new URL("../lib/financial/stripeReconciliation.ts",import.meta.url),"utf8");assert.match(source,/eq\("business_id",input\.businessId\)/);assert.match(source,/stripeAccount:account\.provider_account_id/);assert.match(source,/paymentIntents\.retrieve/);assert.match(source,/amount_received/);assert.match(source,/amount_refunded/);assert.doesNotMatch(source,/\.update\(/);assert.doesNotMatch(source,/\.insert\(/);});
test("Stripe verification distinguishes a missing local ledger payment from a mismatch",()=>{const source=readFileSync(new URL("../lib/financial/stripeReconciliation.ts",import.meta.url),"utf8");assert.match(source,/missing_in_servonas/);assert.match(source,/amount_mismatch/);assert.match(source,/refund_mismatch/);assert.match(source,/duplicate_reference/);});
