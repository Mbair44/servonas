...
test("pest-control website landing page leads with the free website pilot",async()=>{const code=await source();assert.match(code,/We&apos;ll Build Your Pest Control Website/);assert.match(code,/Build My Free Website/);assert.match(code,/First-year domain included/);assert.match(code,/More than a website/);});
...
test("domain copy distinguishes the pilot request from existing-domain connection",async()=>{const code=await source();assert.match(code,/supports connecting a domain you already own/);assert.match(code,/required DNS records/);assert.match(code,/automatic HTTPS after verification/);assert.match(code,/Premium domains are not included/);});
