...
test("promotion code is carried from tenant booking routes through canonical checkout",async()=>{
 ...
 assert.match(client,/const carriedPromotionCode=appliedPromo\?\.code\?\?initialPromotionCode\?\?promoCode\.trim\(\)/);
 ...
 assert.match(client,/applyPromo\(initialPromotionCode,true\)/);
