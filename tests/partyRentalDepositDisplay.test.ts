...
test("blocked rental days still contribute to shared availability state for selected items",async()=>{const source=await readFile(new URL("../components/PartyRentalBookingClient.tsx",import.meta.url),"utf8");assert.match(source,/const blockedDateSet=useMemo\(\(\)=>\{const next=new Set\(blockedDates\);for\(const item of selected\)/);assert.match(source,/capacity===0/);assert.match(source,/setCartConflictMessage\(`Your current cart conflicts with \$\{formatLongDate\(date\)\}/);});
